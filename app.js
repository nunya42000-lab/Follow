// app.js - Core Logic with "Custom" Profile Protection & Fusion Update
...
// --- CONSTANTS ---
const SETTINGS_KEY = 'followMeAppSettings_v11';
const STATE_KEY = 'followMeAppState_v11';
...
function switchToCustomProfile() {
    if (appSettings.activeProfileId === 'custom') return; // Already custom
    
    // Clone current settings into 'custom'
...
function initListeners() {
    document.addEventListener('click', (e) => {
        ...
        if (btn.id === 'camera-master-btn') toggleCamera();
        if (btn.id === 'mic-master-btn') toggleMic();
        if (tab) {
    ...
    // Profile Selection (Quick & Settings)
    const switchProfile = (e) => {
        appSettings.activeProfileId = e.target.value;
        updateAllChrome();
        saveState();
    };
    els.configSelectSettings.onchange = switchProfile;
    els.configSelectQuick.onchange = switchProfile;

    // Profile Management
    els.configAdd.onclick = () => {
    ...
    // Setting changes trigger Custom Protection
    const wrap = (key, val) => modifyProfileSetting(key, val);
    els.modeToggle.onchange = (e) => wrap('currentMode', e.target.checked ? MODES.UNIQUE_ROUNDS : MODES.SIMON);
    els.inputSelect.onchange = (e) => wrap('currentInput', e.target.value);
    els.machineSlider.oninput = (e) => wrap('machineCount', parseInt(e.target.value));
    els.lengthSlider.oninput = (e) => wrap('sequenceLength', parseInt(e.target.value));
    els.autoInputSlider.oninput = (e) => wrap('autoInputMode', String(e.target.value));
    
    // Toggles that affect profile
    const handleAutoplay = (val) => { 
        modifyProfileSetting('isAutoplayEnabled', val); 
        els.quickAutoplay.checked = val; 
        els.settingsAutoplay.checked = val; 
    };
    const handleAudio = (val) => { 
        modifyProfileSetting('isAudioEnabled', val); 
        els.quickAudio.checked = val; 
        els.settingsAudio.checked = val; 
    };

    els.quickAutoplay.onchange = (e) => handleAutoplay(e.target.checked);
    els.settingsAutoplay.onchange = (e) => handleAutoplay(e.target.checked);
    els.quickAudio.onchange = (e) => handleAudio(e.target.checked);
    els.settingsAudio.onchange = (e) => handleAudio(e.target.checked);

    // Global settings (no profile change needed)
    els.themeToggle.onchange = (e) => { appSettings.isDarkMode = e.target.checked; updateAllChrome(); saveState(); };
    els.globalResizeUp.onclick = () => { appSettings.globalUiScale += 10; document.documentElement.style.fontSize = appSettings.globalUiScale + '%'; saveState(); };
    els.globalResizeDown.onclick = () => { appSettings.globalUiScale -= 10; document.documentElement.style.fontSize = appSettings.globalUiScale + '%'; saveState(); };
