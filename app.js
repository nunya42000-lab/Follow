...
(function() {
    ...
    const DEFAULT_PROFILE_SETTINGS = {
        ...
        shakeSensitivity: 10,
        // --- NEW CAMERA SETTINGS ---
        flashSensitivity: 50, // This is now a DELTA value
        cameraGridConfig: {
    ...
    let lastFlashTime = Array(9).fill(0);
    let lastBrightness = Array(9).fill(0);
    ...
    // --- MODAL: Camera ---
    var cameraModal = null, closeCameraModal = null, openCameraModalBtn = null;
    var cameraFeed = null, cameraFeedContainer = null;
    var calibrationGridContainer = null;
    var detectionCanvas = null, detectionContext = null; // <-- NEW
    ...
    function openCameraModal() {
        ...
        const profileSettings = getCurrentProfileSettings();
        const config = profileSettings.cameraGridConfig;
        
        // Apply saved grid settings
        if (calibrationGridContainer) {
            calibrationGridContainer.style.top = config.top;
            calibrationGridContainer.style.left = config.left;
            calibrationGridContainer.style.width = config.width;
            calibrationGridContainer.style.height = config.height;
        }
    ...
    function stopDetection() {
        ...
        if (calibrationGridContainer) {
            for (let i = 0; i < calibrationGridContainer.children.length; i++) {
                calibrationGridContainer.children[i].classList.remove('flash-detected');
            }
        }
    }
    ...
    function runDetectionLoop() {
        if (!isDetecting || !detectionContext || !cameraFeed || !calibrationGridContainer) {
            isDetecting = false;
            return;
        }
        ...
        const gridRect = calibrationGridContainer.getBoundingClientRect();
        ...
        const boxPixelWidth = gridPixelWidth / 3;
        const boxPixelHeight = gridPixelHeight / 3;
        
        const now = Date.now();
        const profileSettings = getCurrentProfileSettings();
        const sensitivity = profileSettings.flashSensitivity; // This is 10-100

        // 4. Analyze each of the 9 boxes
        for (let i = 0; i < 9; i++) {
            ...
            if (delta > sensitivity && (now - lastFlashTime[i] > FLASH_COOLDOWN_MS)) {
                ...
                console.log(`Flash ${value} detected! (Brightness: ${currentBrightness.toFixed(1)}, Delta: ${delta.toFixed(1)})`);
                ...
                const boxEl = calibrationGridContainer.children[i];
                if (boxEl) {
    ...
    function saveGridConfig() {
        if (!calibrationGridContainer || !cameraFeedContainer) return;
        ...
        profileSettings.cameraGridConfig = config;
...
Let's start with Round 1.`;
    ...
    function assignDomElements() {
        ...
        calibrationGridContainer = document.getElementById('calibration-grid-container');
        ...
        // Tab 4: Stealth (Toggles moved here)
    ...
    function initializeListeners() {
        ...
        const addProfileSettingListener = (element, eventType, settingKey, valueType = 'value') => {
            if (element) {
                element.addEventListener(eventType, (e) => {
                    ...
                    if (element === flashSensitivitySlider) { // <-- Sensitivity listener
                        updateFlashSensitivityDisplay(value);
                        saveState(); // Save sensitivity immediately
                    }
        ...
        if (closeCameraModal) closeCameraModal.addEventListener('click', closeCameraModal);
        if (startCameraBtn) startCameraBtn.addEventListener('click', startCameraStream);
        if (startDetectionBtn) startDetectionBtn.addEventListener('click', startDetection); // <-- NEW
        if (stopDetectionBtn) stopDetectionBtn.addEventListener('click', stopDetection); // <-- NEW
        ...
        if (calibrationGridContainer) {
            ...
            const dragStart = (e) => {
                ...
                startLeft = calibrationGridContainer.offsetLeft;
                startTop = calibrationGridContainer.offsetTop;
            ...
            const dragMove = (e) => {
                ...
                calibrationGridContainer.style.left = `${(newLeft / parentRect.width) * 100}%`;
                calibrationGridContainer.style.top = `${(newTop / parentRect.height) * 100}%`;
            };
            ...
            // Listen for drag
            calibrationGridContainer.addEventListener('mousedown', dragStart);
            calibrationGridContainer.addEventListener('touchstart', dragStart);

            // Listen for resize (which also ends with a mouseup)
            // Note: This relies on the 'resize: both' CSS property.
            const resizeObserver = new ResizeObserver(() => {
                if (!isDraggingGrid) {
                    saveGridConfig();
                }
            });
            resizeObserver.observe(calibrationGridContainer);
        }
