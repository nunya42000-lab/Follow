...
(function() {
    ...
    // --- SKILL VISION ENGINE 2.1 SETTINGS ---
    const ENGINE_THRESHOLD = 40;   // Sensitivity threshold
    const ADAPTIVE_FACTOR = 0.2;   // How fast the baseline adjusts (0.2 = 20% new frame, 80% history)
    const FLASH_DEBOUNCE_MS = 300; // Global debounce between hits
    ...
    const DEFAULT_PROFILE_SETTINGS = {
        ...
        flashSensitivity: 50, // Kept for slider UI, though Engine 2.1 uses const currently
    ...
    // --- NEW ENGINE STATE ---
    let lastHitTime = 0;
    // Using size 12 to accommodate 9-key or 12-key grids
    ...
    function handleConfigDelete() {
        ...
        showModal("Delete?", "Permanently delete this configuration?", () => {
            const pid = appSettings.activeProfileId;
            delete appSettings.profiles[pid];
            delete appState[pid];
    ...
    // =================================================================
    // === SKILL VISION ENGINE 2.1 (NEW IMPLEMENTATION) ===
    // =================================================================
    ...
    function startDetection() {
        ...
        // Reset baseline to zeroes. This allows the very first frame to 
        // act as the initial baseline or quickly adapt.
        baselineData.fill(0); 
        lastHitTime = 0;
    ...
    function runDetectionLoop() {
        ...
        // 4. Engine Core: Center Point Sampling & Adaptive Baseline
        for (let i = 0; i < numBoxes; i++) {
            ...
            // === KEY CHANGE: Center Point Sampling ===
            // Instead of averaging the whole area, we sample the exact center pixel.
            const centerX = Math.floor(boxX + (boxPixelWidth / 2));
            const centerY = Math.floor(boxY + (boxPixelHeight / 2));

            // Boundary check
            if (centerX < 0 || centerX >= detectionCanvas.width || centerY < 0 || centerY >= detectionCanvas.height) continue;

            const pixelIndex = (centerY * detectionCanvas.width + centerX) * 4;
            ...
            // Calculate difference from running baseline
            const diff = Math.abs(currentBrightness - baselineData[i]);

            // Trigger Check
            if (diff > ENGINE_THRESHOLD) {
                
                // Debounce
                if (now - lastHitTime > FLASH_DEBOUNCE_MS) {
                    ...
                    // Actual Logic Trigger
                    if (isCameraMasterOn) {
                        console.log(`SV2.1: Detected ${value}`);
                        addValue(value);
                    } else {
                        console.log(`SV2.1: Detected ${value} (Calibration)`);
                    }
            ...
            // Formula: NewBaseline = (OldBaseline * 0.8) + (CurrentBrightness * 0.2)
            // This allows the system to adapt to slow lighting changes while catching fast flashes.
            baselineData[i] = (baselineData[i] * (1 - ADAPTIVE_FACTOR)) + (currentBrightness * ADAPTIVE_FACTOR);
        }
    ...
    function generateGeneralHelp() {
        ...
        if (container) container.innerHTML = `<h4 class="text-primary-app">App Overview</h4><p>This is a multi-mode sequence tracker with customizable profiles.</p><p>Use the **Quick Start** modal (on launch) to select your saved **Configuration Profile**. You can add, rename, or delete profiles from there.</p><p>All detailed settings are in the **Settings (⚙️)** menu, organized into tabs: "Profile", "Global", "Shortcuts", and "Stealth".</p><h4 class="text-primary-app">Basic Controls</h4><ul><li><span class="font-bold">Keypad:</span> Tap the numbers or keys to add to the sequence.</li><li><span class="font-bold">Play (▶):</span> Plays back the current sequence(s).</li><li><span class="font-bold">Backspace (←):</span> Removes the last value.</li><li><span class="font-bold">Settings (⚙️):</span> Opens app preferences.</li><li><span class="font-bold">RESET:</span> (Unique Rounds Mode Only) Resets the game to Round 1.</li></ul>`;
    }
    function generateModesHelp() {
        const container = document.getElementById('help-tab-modes');
        if (container) container.innerHTML = `<h4 class="text-primary-app">Inputs (The Keypads)</h4><p>Set in Settings ➔ Profile Tab</p><ul><li><span class="font-bold">9-Key:</span> A 3x3 grid (1-9).</li><li><span class="font-bold">12-Key:</span> A 4x3 grid (1-12).</li><li><span class="font-bold">Piano:</span> A 7-key piano with 5 sharps.</li></ul><h4 class="text-primary-app">Modes (The Logic)</h4><p>Set in Settings ➔ Profile Tab</p><ul><li><span class="font-bold">Simon Says (Default):</span> Standard mode. Enter values up to the Sequence Length. Supports 1-4 machines.</li><li><span class="font-bold">Unique Rounds:</span> A round-based game. The sequence length increases by one each round, up to the Sequence Length (15, 20, or 25). This mode is always single-machine.</li></ul>`;
    }
