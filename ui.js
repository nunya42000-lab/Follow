...
export function renderUI() {
    ...
    if(appSettings.isPracticeModeEnabled) {
        ...
        if(practiceSequence.length === 0) { 
            ...
            replayBtn.onclick = () => {
                setPracticeInputIndex(0); 
                showToast("Replaying... 👂", appSettings);
            ...
            resetLvlBtn.onclick = () => {
                if(confirm("Restart practice from Level 1?")) {
                    // Reset array in core by calling startPracticeRound logic or a core reset func
                    state.currentRound = 1;
                    renderUI(); // Re-render to show start button
                }
            };
            ...
            if (settings.currentMode !== CONFIG.MODES.UNIQUE_ROUNDS) {
                ...
                btnTrash.onclick = (e) => {
                    ...
                    if(confirm(`Remove Machine ${idx + 1} entirely?`)) {
                        ...
                        vibrate(appSettings);
                        showToast(`Removed Machine ${idx + 1}`, appSettings);
                        renderUI();
                        saveState();
                    }
                };
