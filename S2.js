...
export const DEFAULT_MAPPINGS = {
    ...
    key12: {
        ...
        '10': { g: 'swipe_down_3', m: '--.' },
        '11': { g: 'swipe_up_3', m: '--..' },
        '12': { g: 'swipe_right_3', m: '---' }
    },
    piano: {
        '1': { g: 'swipe_left_2', m: '-' },
        '2': { g: 'swipe_upleft_2', m: '-.' },
        '3': { g: 'swipe_up_2', m: '--' },
        '4': { g: 'swipe_upright_2', m: '-..' },
        '5': { g: 'swipe_right_2', m: '-.-' },
        'C': { g: 'swipe_upleft', m: '.' },
        'D': { g: 'swipe_left', m: '..' },
        'E': { g: 'swipe_downleft', m: '.-' },
        'F': { g: 'swipe_down', m: '...' },
        'G': { g: 'swipe_downright', m: '..-' },
        'A': { g: 'swipe_right', m: '.-.' },
        'B': { g: 'swipe_upright', m: '.--' }
    }
};
...
const LANG = {
    en: {
        ...
        inputs_only: "Inputs Only (Hold 1)", gestures_enable: "Gestures", timer_enable: "Timer", counter_enable: "Counter",
        morse_pause: "Morse Pause", black_gestures: "Blackout Gestures",
        help_stealth_detail: "Inputs Only (1-Key) simplifies input by mapping the 12 primary values (1-12) to a single key press.",
        help_blackout_detail: "Blackout Mode turns the entire screen black. Shake to wake.",
        help_gesture_detail: "Blackout Gestures allow no-look input via swipes and taps."
    },
    es: {
        ...
        inputs_only: "Solo Entradas (Manten 1)", gestures_enable: "Gestos", timer_enable: "Temporizador", counter_enable: "Contador",
        morse_pause: "Pausa Morse", black_gestures: "Gestos de Pantalla Negra",
        help_stealth_detail: "El modo Solo Entradas simplifica la entrada al asignar los valores a una sola tecla.",
        help_blackout_detail: "El modo de pantalla negra oscurece toda la pantalla. Agitar para despertar.",
        help_gesture_detail: "Los gestos permiten la entrada sin mirar."
    }
};
...
export class SettingsManager {
    constructor(appSettings, callbacks, sensorEngine) {
        ...
        // Ensure mapping data exists
        if (!this.appSettings.inputMappings) {
            this.appSettings.inputMappings = JSON.parse(JSON.stringify(DEFAULT_MAPPINGS));
        }

        // 1. Inject elements first (creates them in the DOM if missing)
        // We will now rely on render in app.js or dynamic injection for new toggles if not in HTML
        ...
        this.dom = {
            ...
            // New General Toggles
            gesturesToggle: document.getElementById('gestures-toggle'),
            timerToggle: document.getElementById('timer-toggle'),
            counterToggle: document.getElementById('counter-toggle'),

            // Stealth Tab Elements
            stealth1KeyToggle: document.getElementById('stealth-1key-toggle'), // Renamed "Inputs Only" in UI
            ...
            morsePause: document.getElementById('morse-pause-select'),
            
            // Mapping Containers
            mapContainer9: document.getElementById('map-container-9'),
            mapContainer12: document.getElementById('map-container-12'),
            mapContainerPiano: document.getElementById('map-container-piano'),

            // Injected Toggles (Legacy support or if generated dynamically)
            ...
            btnChimeMain: document.getElementById('btn-chime-main'), // New Chime Button
        ...
        // Inject new UI elements if they don't exist in HTML yet
        this.injectNewSettingsUI();
        
        // Re-cache specific elements after injection
        this.dom.stealth1KeyToggle = document.getElementById('stealth-1key-toggle');
        this.dom.blackoutToggle = document.getElementById('blackout-toggle');
        this.dom.blackoutGesturesToggle = document.getElementById('blackout-gestures-toggle');
        this.dom.hapticMorse = document.getElementById('haptic-morse-toggle');
        this.dom.morsePause = document.getElementById('morse-pause-select');
        this.dom.gesturesToggle = document.getElementById('gestures-toggle');
        this.dom.timerToggle = document.getElementById('timer-toggle');
        this.dom.counterToggle = document.getElementById('counter-toggle');
        this.dom.mapContainer9 = document.getElementById('map-container-9');
        this.dom.mapContainer12 = document.getElementById('map-container-12');
        this.dom.mapContainerPiano = document.getElementById('map-container-piano');

        this.initListeners(); this.populateConfigDropdown(); this.populateThemeDropdown(); this.buildColorGrid(); this.populateVoicePresetDropdown(); this.populateMappingInputs();
    }
    injectNewSettingsUI() {
        // 1. ADD STEALTH TAB BUTTON
        const tabContainer = document.querySelector('.tab-btn').parentElement;
        if (!document.querySelector('[data-tab="stealth"]')) {
            const stealthTab = document.createElement('div');
            stealthTab.className = "tab-tab-btn tab-btn"; // using existing class structure
            stealthTab.dataset.tab = "stealth";
            stealthTab.textContent = LANG[this.appSettings.generalLanguage || 'en'].lbl_stealth;
            // Insert before General or append
            tabContainer.appendChild(stealthTab);
            
            // Re-bind tab listeners since we added one
            this.dom.tabs = document.querySelectorAll('.tab-btn');
        }

        // 2. CREATE STEALTH CONTENT CONTAINER
        let stealthContent = document.getElementById('tab-stealth');
        if (!stealthContent) {
            stealthContent = document.createElement('div');
            stealthContent.id = "tab-stealth";
            stealthContent.className = "tab-content";
            
            // Build Structure
            stealthContent.innerHTML = `
                <div class="space-y-3">
                    <div class="grid grid-cols-2 gap-3">
                        <div class="flex flex-col p-3 rounded-lg settings-input border border-custom">
                        ...
                        <div class="flex flex-col p-3 rounded-lg settings-input border border-custom">
                             <label class="font-bold text-xs mb-1" data-i18n="morse_pause">Morse Pause</label>
                             <select id="morse-pause-select" class="bg-black bg-opacity-30 rounded p-1 text-sm">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                            <span class="font-bold text-sm text-xs">Blackout Mode</span>
                            <input type="checkbox" id="blackout-toggle" class="h-5 w-5 accent-indigo-500">
                        </div>
                        <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                            <span class="font-bold text-[10px]" data-i18n="black_gestures">Blackout Gestures</span>
                            <input type="checkbox" id="blackout-gestures-toggle" class="h-5 w-5 accent-indigo-500">
                        </div>
                    </div>

                    <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                        <span class="font-bold text-sm" data-i18n="inputs_only">Inputs Only (Hold 1)</span>
                    ...
                    <div class="space-y-4">
                        <div>
                            <h4 class="font-bold text-primary-app mb-2">9-Key Mapping</h4>
                            <div id="map-container-9" class="space-y-2 text-xs"></div>
                        </div>
                        <div>
                            <h4 class="font-bold text-primary-app mb-2">12-Key Mapping</h4>
                            <div id="map-container-12" class="space-y-2 text-xs"></div>
                        </div>
                        <div>
                            <h4 class="font-bold text-primary-app mb-2">Piano Mapping</h4>
                            <div id="map-container-piano" class="space-y-2 text-xs"></div>
                        </div>
                    </div>
                </div>
            `;
            ...
            const container = document.getElementById('tab-general').parentElement;
            container.appendChild(stealthContent);
        }

        // 3. REORGANIZE GENERAL TAB (2 per row)
        const genTab = document.getElementById('tab-general');
        if (genTab) {
            // Clear existing simple list to rebuild with grid
            const uiScaleDiv = document.getElementById('ui-scale-select')?.parentElement?.parentElement; // Preserve Scale/Theme
            
            // We reconstruct the top part
            const togglesHTML = `
                <div class="grid grid-cols-2 gap-3 mb-4">
                    <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                        <span class="font-bold text-sm">Haptics</span>
                        <input type="checkbox" id="haptics-toggle" class="h-5 w-5 accent-indigo-500">
                    </div>
                    <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                        <span class="font-bold text-sm">Speed Del</span>
                        <input type="checkbox" id="speed-delete-toggle" class="h-5 w-5 accent-indigo-500">
                    </div>
                    <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                        <span class="font-bold text-sm" data-i18n="gestures_enable">Gestures</span>
                        <input type="checkbox" id="gestures-toggle" class="h-5 w-5 accent-indigo-500">
                    </div>
                    <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                        <span class="font-bold text-sm" data-i18n="timer_enable">Timer</span>
                        <input type="checkbox" id="timer-toggle" class="h-5 w-5 accent-indigo-500">
                    </div>
                    <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                        <span class="font-bold text-sm" data-i18n="counter_enable">Counter</span>
                        <input type="checkbox" id="counter-toggle" class="h-5 w-5 accent-indigo-500">
                    </div>
                    <div class="flex justify-between items-center p-3 rounded-lg settings-input border border-custom">
                        <span class="font-bold text-xs">Quick Start</span>
                        <input type="checkbox" id="show-welcome-toggle" class="h-5 w-5 accent-indigo-500">
                    </div>
                </div>
            `;
            
            // Prepend toggles, keep the rest (Theme, Scale, etc)
            // We need to be careful not to destroy the listeners of existing elements if we nuke innerHTML.
            // However, since we re-init listeners in constructor, we can overwrite HTML structure here safely IF this runs before initListeners.
            // To be safe, we insert 'togglesHTML' at the top of genTab and hide/remove old duplicates.
            
            // Strategy: Create a wrapper for toggles, insert as first child.
            // Hide specific old IDs to avoid ID conflicts or visual duplicates if they exist in HTML.
            
            // Actually, simpler: Set innerHTML of a new top div.
            const topDiv = document.createElement('div');
            topDiv.innerHTML = togglesHTML;
            if (genTab.firstChild) {
                genTab.insertBefore(topDiv, genTab.firstChild);
            } else {
                genTab.appendChild(topDiv);
            }
            
            // Remove old specific containers if they exist to avoid dupes
            // (Assumes original HTML had them in simple divs. We just hide them via CSS or logic if needed, 
            // but since we are re-binding by ID, the new IDs in topDiv will take precedence if we querySelect properly)
        }

        // 4. REORGANIZE PLAYBACK TAB
        const playTab = document.getElementById('tab-playback');
        if (playTab) {
            const container = playTab.querySelector('.space-y-3');
            if (container) {
                // Find Autoplay and Audio and wrap them
                // This is hard to do purely with JS selectors without breaking event listeners if we move them.
                // Alternative: Inject a new Grid container at the top and move the elements into it.
                const grid = document.createElement('div');
                grid.className = "grid grid-cols-2 gap-3";
                
                const apDiv = document.getElementById('autoplay-toggle')?.parentElement;
                const auDiv = document.getElementById('audio-toggle')?.parentElement;
                
                if (apDiv && auDiv) {
                    grid.appendChild(apDiv);
                    grid.appendChild(auDiv);
                    container.insertBefore(grid, container.firstChild);
                }
            }
        }

        // 5. INJECT CHIME BUTTON INTO DONATE MODAL
        const cashBtn = document.getElementById('btn-cashapp-main');
        if (cashBtn && !document.getElementById('btn-chime-main')) {
            const chimeBtn = document.createElement('button');
            chimeBtn.id = "btn-chime-main";
            chimeBtn.className = "w-full bg-[#3D4E53] hover:bg-[#2c383c] text-white font-bold py-3 rounded-lg mb-3 shadow-lg flex items-center justify-center text-lg border border-[#00C896]";
            chimeBtn.innerHTML = `<span class="mr-2">🏦</span> Chime ($jwo83)`;
            cashBtn.parentElement.appendChild(chimeBtn); // Add to same container
        }
    }

    populateMappingInputs() {
        const createRow = (key, data, type) => {
            const row = document.createElement('div');
            row.className = "flex items-center space-x-2 bg-black bg-opacity-20 p-2 rounded";
            
            // Label
            const lbl = document.createElement('span');
            lbl.className = "font-bold w-6 text-center";
            lbl.textContent = key;
            
            // Gesture Select
            const sel = document.createElement('select');
            sel.className = "bg-gray-800 text-[10px] rounded p-1 w-24 border border-gray-600";
            const gestures = [
                "tap", "dbl_tap", "long_press", 
                "tap_2", "dbl_tap_2", "long_press_2", 
                "tap_3", "dbl_tap_3", "long_press_3",
                "swipe_left", "swipe_right", "swipe_up", "swipe_down",
                "swipe_left_2", "swipe_right_2", "swipe_up_2", "swipe_down_2",
                "swipe_left_3", "swipe_right_3", "swipe_up_3", "swipe_down_3",
                "swipe_upleft", "swipe_upright", "swipe_downleft", "swipe_downright",
                "swipe_upleft_2", "swipe_upright_2", "swipe_downleft_2", "swipe_downright_2"
            ];
            
            // Sort gestures alphabetically for easier finding
            // gestures.sort(); 
            ...
            gestures.forEach(g => {
                ...
                if (data.g === g) opt.selected = true;
                sel.appendChild(opt);
            });
            
            sel.onchange = (e) => {
                this.appSettings.inputMappings[type][key].g = e.target.value;
            ...
            inp.type = "text";
            inp.className = "bg-gray-800 text-[10px] rounded p-1 flex-grow border border-gray-600 font-mono tracking-widest text-center";
            inp.value = data.m;
            inp.onchange = (e) => {
                this.appSettings.inputMappings[type][key].m = e.target.value;
                this.callbacks.onSave();
            };
        ...
        // Populate 9 Key
        if (this.dom.mapContainer9) {
            this.dom.mapContainer9.innerHTML = '';
            Object.keys(this.appSettings.inputMappings.key9).forEach(k => {
                this.dom.mapContainer9.appendChild(createRow(k, this.appSettings.inputMappings.key9[k], 'key9'));
            });
        }
        // Populate 12 Key
        if (this.dom.mapContainer12) {
            this.dom.mapContainer12.innerHTML = '';
            // Sort numeric
            const keys = Object.keys(this.appSettings.inputMappings.key12).sort((a,b) => parseInt(a)-parseInt(b));
            keys.forEach(k => {
                this.dom.mapContainer12.appendChild(createRow(k, this.appSettings.inputMappings.key12[k], 'key12'));
            });
        }
        // Populate Piano
        if (this.dom.mapContainerPiano) {
            this.dom.mapContainerPiano.innerHTML = '';
            // Custom sort for piano
            const order = ['1','2','3','4','5','C','D','E','F','G','A','B'];
            order.forEach(k => {
                 if (this.appSettings.inputMappings.piano[k]) {
                     this.dom.mapContainerPiano.appendChild(createRow(k, this.appSettings.inputMappings.piano[k], 'piano'));
                 }
            });
        }
    }
    ...
    initListeners() {
        // ... (Existing Listeners from Part 1 Logic) ...
        // Re-implementing standard listeners + new ones
        ...
        if (this.dom.voicePresetSave) this.dom.voicePresetSave.onclick = () => {
            ...
            if (PREMADE_VOICE_PRESETS[id]) { alert("Cannot save over built-in presets. Create a new one."); return; }
        ...
        if (this.dom.voicePresetRename) this.dom.voicePresetRename.onclick = () => {
            ...
            if (n) { this.appSettings.voicePresets[id].name = n; this.populateVoicePresetDropdown(); this.callbacks.onSave(); }
        };
        ...
        const bind = (el, prop, isGlobal, isInt = false, isFloat = false) => {
            ...
            el.onchange = () => {
                ...
                if (isGlobal) {
                    ...
                    if (['isGesturesEnabled', 'isTimerEnabled', 'isCounterEnabled'].includes(prop)) this.callbacks.onUpdate();
                } else {
        ...
        bind(this.dom.morsePause, 'morsePauseDuration', true, true);
        bind(this.dom.gesturesToggle, 'isGesturesEnabled', true);
        bind(this.dom.timerToggle, 'isTimerEnabled', true);
        bind(this.dom.counterToggle, 'isCounterEnabled', true);
        bind(this.dom.stealth1KeyToggle, 'isStealth1KeyEnabled', true);
        ...
        if (this.dom.btnChimeMain) this.dom.btnChimeMain.onclick = () => { window.open('https://member.chime.com/pay-anyone/jwo83', '_blank'); };
    ...
    generatePrompt() {
        ...
        if (machines > 1) {
            instructions = `MODE: MULTI-MACHINE AUTOPLAY (${machines} Machines). Sort them: 1->M1, 2->M2. Read back interleaved chunks of ${chunk}. Pause ${delay}s between machine switches.`;
        } else {
            if (ps.currentMode === 'simon') {
                instructions = `MODE: SIMON SAYS. Seq grows by 1. Read entire list.`;
            } else {
                instructions = `MODE: UNIQUE. Random seq. Repeat number. If "Review", read all.`;
            }
        }

        const promptText = `Act as Sequence Caller. Max Num: ${max}. Speed: ${speed}x. Machines: ${machines}. ${instructions} START NOW.`;
    ...
    updateUIFromSettings() {
        ...
        if (this.dom.gesturesToggle) this.dom.gesturesToggle.checked = this.appSettings.isGesturesEnabled;
        ...
        if (this.dom.morsePause) this.dom.morsePause.value = this.appSettings.morsePauseDuration || 0;
