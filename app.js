...
const DEFAULT_APP = { 
    ...
    // --- NEW SETTINGS ---
    isFlashEnabled: true, // Controls visual flashing during playback
    hapticPause: 0.0, // Extra pause between inputs during haptic playback (seconds)
    // --------------------
...
function loadState() { 
    try { 
        ...
        if(s) { 
            ...
            if (typeof appSettings.isFlashEnabled === 'undefined') appSettings.isFlashEnabled = true;
            if (typeof appSettings.hapticPause === 'undefined') appSettings.hapticPause = 0.0;
            // -----------------------------
...
function playDemo() {
    ...
    function nextChunk() {
        ...
        function playNum() {
            ...
            // --- UPDATED LOGIC: ONLY FLASH IF ENABLED ---
            ...
            // --- UPDATED LOGIC: HAPTIC PAUSE DELAY ---
            let delay = (CONFIG.DEMO_DELAY_BASE_MS / speed);
            if(appSettings.isHapticMorseEnabled && appSettings.hapticPause > 0) {
                // Add pause (stored in seconds, convert to ms)
                delay += (appSettings.hapticPause * 1000);
            }
            
            setTimeout(playNum, delay);
        }
        playNum();
    }
    nextChunk();
