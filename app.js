...
(function() {
    ...
    const MIC_THRESHOLD = 20; // Magnitude threshold (volume gate)
    ...
    // --- TONE MAPPING (Standard Chromatic Scale C4-C5) ---
    // You can adjust these ranges if the machine is lower/higher
    const NOTE_MAP = [
        { note: 'C',  freq: 261.63, val: '1' },
        { note: 'C#', freq: 277.18, val: '2' },
        { note: 'D',  freq: 293.66, val: '3' },
        { note: 'D#', freq: 311.13, val: '4' },
        { note: 'E',  freq: 329.63, val: '5' },
        { note: 'F',  freq: 349.23, val: '6' },
        { note: 'F#', freq: 369.99, val: '7' },
        { note: 'G',  freq: 392.00, val: '8' },
        { note: 'G#', freq: 415.30, val: '9' },
        { note: 'A',  freq: 440.00, val: '1' }, // Rollover 1
        { note: 'A#', freq: 466.16, val: '2' }, // Rollover 2
        { note: 'B',  freq: 493.88, val: '3' }  // Rollover 3
    ];
    ...
    let baselineBrightness = Array(12).fill(0); 
    ...
    let audioProcessLoopId = null;
    let isMicMasterOn = false;
    let lastAudioHitTime = 0;
        ...
        try {
            ...
            if (storedSettings) {
                ...
                Object.keys(appSettings.profiles).forEach(profileId => {
                    appSettings.profiles[profileId].settings = { ...DEFAULT_PROFILE_SETTINGS, ...appSettings.profiles[profileId].settings };
                });
            } else {
            ...
            appState = {};
            Object.keys(appSettings.profiles).forEach(id => appState[id] = getInitialState());
        }
    }
    ...
    function populateConfigDropdown() {
        ...
        Object.keys(appSettings.profiles).forEach(profileId => {
            ...
            option.value = profileId; option.textContent = appSettings.profiles[profileId].name;
            configSelect.appendChild(option);
        });
        configSelect.value = appSettings.activeProfileId;
    }
    function switchActiveProfile(newProfileId) { if (appSettings.profiles[newProfileId]) { appSettings.activeProfileId = newProfileId; updateAllChrome(); saveState(); } }
    function handleConfigAdd() {
        const newName = prompt("Enter new configuration name:", "New Setup");
        if (name) {
            const id = `profile_${Date.now()}`;
            appSettings.profiles[id] = { name: newName, settings: { ...DEFAULT_PROFILE_SETTINGS } };
            appState[id] = getInitialState();
            switchActiveProfile(id); populateConfigDropdown();
        }
    }
    function handleConfigRename() {
        const newName = prompt("Enter new name:", appSettings.profiles[appSettings.activeProfileId].name);
        if (newName) { appSettings.profiles[appSettings.activeProfileId].name = newName; populateConfigDropdown(); saveState(); }
    }
    function handleConfigDelete() {
        if (Object.keys(appSettings.profiles).length <= 1) return showModal("Cannot Delete", "You must have at least one configuration.", closeModal, "OK", "");
    ...
    // ==========================================================
    // V3 CAMERA ENGINE (MOTION/ADAPTIVE)
    // ==========================================================
    function startDetection() {
        if (isDetecting || !cameraStream || !detectionCanvas) return;
        isDetecting = true;
        baselineBrightness.fill(0); lastFlashTime.fill(0);
    ...
    function runDetectionLoop() {
        ...
        for (let i = 0; i < numBoxes; i++) {
            ...
            if (baselineBrightness[i] === 0) baselineBrightness[i] = bright;
            const delta = Math.abs(bright - baselineBrightness[i]);

            if (delta > sensitivity && (now - lastFlashTime[i] > FLASH_COOLDOWN_MS)) {
                const val = String(i + 1);
                if (isCameraMasterOn) addValue(val);
                lastFlashTime[i] = now;
            ...
            baselineBrightness[i] = (baselineBrightness[i] * (1 - ADAPTIVE_LEARNING_RATE)) + (bright * ADAPTIVE_LEARNING_RATE);
        }
        if (isDetecting) detectionLoopId = requestAnimationFrame(runDetectionLoop);
    }

    // ==========================================================
    // AUDIO ENGINE V1 (FREQUENCY ANALYSIS)
    // ==========================================================
    function toggleMicMaster() {
        isMicMasterOn = !isMicMasterOn;
        updateMasterButtonStates();
        if (isMicMasterOn) startAudioListening(); else stopAudioListening();
    }
    ...
    async function startAudioListening() {
        ...
        try {
            ...
            showModal("Microphone Error", "Could not access microphone. Check permissions.", closeModal, "OK", "");
            isMicMasterOn = false; updateMasterButtonStates();
        }
    }
    ...
    function stopAudioListening() {
        ...
        if (audioProcessLoopId) cancelAnimationFrame(audioProcessLoopId);
        showToast("Mic Off");
    }
    ...
    function runAudioLoop() {
        ...
        // Gate: Must be loud enough
        if (maxVal > MIC_THRESHOLD) {
            const nyquist = audioContext.sampleRate / 2;
            const frequency = maxIndex * (nyquist / bufferLength);
            
            // Match to Note Map
            if (frequency > 250 && frequency < 550) { // Filter reasonable range
                const match = NOTE_MAP.find(n => Math.abs(frequency - n.freq) < 8); // +/- 8Hz tolerance
                const now = Date.now();
                if (match && (now - lastAudioHitTime > AUDIO_DEBOUNCE_MS)) {
                    console.log(`Heard: ${frequency.toFixed(1)}Hz -> ${match.note} (${match.val})`);
                    addValue(match.val);
                    lastAudioHitTime = now;
                }
            }
        }
    ...
    function speak(text) { 
        ...
        u.rate = 1.2; 
        window.lastUtterance = u; // Prevent Garbage Collection Bug
    ...
    function disableKeys(disable) { document.querySelectorAll(`#pad-${getCurrentProfileSettings().currentInput} button[data-value]`).forEach(k => k.disabled = disable); }
    ...
    function handleSimonDemo() {
        ...
        if (maxLen === 0 || (demoBtn && demoBtn.disabled)) return;
        const playlist = [];
        const chunkSize = (p.machineCount > 1) ? p.simonChunkSize : maxLen;
        for (let c = 0; c < Math.ceil(maxLen / chunkSize); c++) {
        ...
        demoBtn.disabled = true; disableKeys(true);
        let i = 0;
        ...
        function playNext() {
            if (i < playlist.length) {
                ...
                i++;
            } else { demoBtn.disabled = false; demoBtn.innerHTML = '▶'; disableKeys(false); renderSequences(); }
        }
        playNext();
    }
    function handleUniqueRoundsDemo() {
        ...
        const demoBtn = document.querySelector(`${pad} button[data-action="play-demo"]`);
        if (!seq.length || (demoBtn.disabled && !p.isUniqueRoundsAutoClearEnabled)) return;
        demoBtn.disabled = true; disableKeys(true);
        let i = 0;
        const speed = appSettings.playbackSpeed;
        const flashDur = 250 / (speed > 1 ? speed : 1);
        ...
        function playNext() {
            if (i < seq.length) {
                ...
                demoBtn.innerHTML = String(i+1);
                speak(input === INPUTS.PIANO ? PIANO_SPEAK_MAP[val] || val : val);
                if (key) { key.classList.add(flashCls); setTimeout(() => { key.classList.remove(flashCls); setTimeout(playNext, pauseDur - flashDur); }, flashDur); } 
                else setTimeout(playNext, pauseDur);
                i++;
            } else {
                demoBtn.disabled = false; demoBtn.innerHTML = '▶';
                if (p.isUniqueRoundsAutoClearEnabled) setTimeout(clearUniqueRoundsSequence, 300); else disableKeys(false);
            }
        }
        playNext();
    }
    function clearUniqueRoundsSequence() {
        ...
        speedDeleteInterval = setInterval(() => {
            if (s.sequences[0].length > 0) { s.sequences[0].pop(); s.nextSequenceIndex--; renderSequences(); }
                ...
                clearInterval(speedDeleteInterval); s.currentRound++; 
    ...
    function executeShortcut(trig) {
        ...
        switch (sc.action) {
            ...
            case 'toggle_autoplay': s.isAutoplayEnabled = !s.isAutoplayEnabled; updateSettingsModalVisibility(); showToast(`Autoplay: ${s.isAutoplayEnabled?'On':'Off'}`); break;
            case 'toggle_audio': s.isAudioEnabled = !s.isAudioEnabled; updateSettingsModalVisibility(); showToast(`Audio: ${s.isAudioEnabled?'On':'Off'}`); break;
            case 'toggle_haptics': s.isHapticsEnabled = !s.isHapticsEnabled; updateSettingsModalVisibility(); showToast(`Haptics: ${s.isHapticsEnabled?'On':'Off'}`); break;
    ...
    function initializeListeners() {
        // MIC LISTENER - WIRED TO AUDIO ENGINE
        document.body.addEventListener('click', (e) => {
            if (e.target.closest('#mic-master-btn')) { toggleMicMaster(); } // FIXED: Calls toggleMicMaster
        ...
        document.querySelectorAll('button[data-action="backspace"]').forEach(btn => {
            btn.addEventListener('mousedown', () => { stopSpeedDeleting(); initialDelayTimer = setTimeout(() => { isHoldingBackspace = true; if (!executeShortcut('longpress_backspace')) speedDeleteInterval = setInterval(handleBackspace, SPEED_DELETE_INTERVAL_MS); }, SPEED_DELETE_INITIAL_DELAY); });
            btn.addEventListener('mouseup', () => { stopSpeedDeleting(); if (!isHoldingBackspace) handleBackspace(); isHoldingBackspace = false; });
        ...
        const addSettingListener = (el, key, type='value') => {
            if (el) el.addEventListener('input', (e) => {
                ...
                if (type==='int') val = parseInt(val) || 0; // FIXED: Prevents NaN crash
