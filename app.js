...
// --- FIXED: Standardized 'unique_rounds' to 'unique' to match HTML values ---
...
function loadState() { 
    try { 
        ...
        if(s) { 
            ...
            // --- FIXED: Migration Script for old 'unique_rounds' value ---
            ...
            Object.values(appSettings.profiles).forEach(p => {
                if(p.settings.currentMode === 'unique_rounds') p.settings.currentMode = 'unique';
            });
        } else { 
