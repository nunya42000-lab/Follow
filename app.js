...
let retryCount = 0; // Fix for Infinite Loop
let lastTiltTime = 0; // For Tilt throttling
...
(function() {
    ...
    async function initApp() {
        ...
        // Fix: Infinite Loop & Listener Leak
        // Check if DOM is ready. If not, retry a limited number of times.
        if (!DOM.sequenceContainer) {
            if (retryCount < 10) {
                console.warn(`DOM elements not found. Retrying init (${retryCount + 1}/10)...`);
                retryCount++;
                setTimeout(initApp, 100); // Increased delay slightly
            } else {
                console.error("Failed to initialize app: DOM elements missing.");
            }
            return;
        }
        ...
        // Initialize Listeners ONLY once DOM is ready
        initializeListeners();
    ...
    function initializeListeners() {
        // --- MASTER SWITCHES (Handles clicks on the Camera/Mic Icons) ---
        ...
        document.addEventListener('click', (event) => {
            ...
            if (action === 'open-help') { 
                UI.closeModal(); 
                // Fix: Ensure tab defaults to general so it's not empty
        ...
        // --- SWIPE GESTURES (New Implementation) ---
    ...
    // Fix: New Tilt Handler
    function handleTilt(event) {
        const now = Date.now();
        if (now - lastTiltTime < 500) return; // Throttle tilt events
        
        const gamma = event.gamma; // Left to Right tilt
        const beta = event.beta;   // Front to Back tilt (optional, but we are using gamma for L/R)
    ...
    function openHelpModal() {
        ...
        // Tab switched in event listener now
