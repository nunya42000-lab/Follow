// gestures.js
// Version: v68 - Fixed Spatial Taps & Shake Events
...
export class GestureEngine {
    ...
    _handleDown(e) {
        ...
        this.activePointers[e.pointerId] = {
            id: e.pointerId, pts: [{ x: e.clientX, y: e.clientY }], startTime: Date.now()
        };

        const count = Object.keys(this.activePointers).length;
        const pointers = Object.values(this.activePointers);

        // Init Squiggle (Shake) Detection
        if (count === 1) {
            this.contState.squiggle = { isTracking: true, startX: e.clientX, lastX: e.clientX, direction: 0, flips: 0, hasTriggered: false };
        }
        if (count === 2) {
            const p1 = pointers[0].pts[0], p2 = pointers[1].pts[0];
            ...
            this.contState.pinchStartDist = Math.hypot(p1.x - p2.x, p1.y - p2.y);
            this.contState.squiggle2F = { isTracking: true, lastX: (p1.x + p2.x) / 2, direction: 0, flips: 0, hasTriggered: false };
        }
    }

    _handleMove(e) {
        if (!this.activePointers[e.pointerId]) return;
        // Prevent browser back/scroll if tracking gestures
        ...
        if (count === 1 && this.contState.squiggle.isTracking && !this.contState.squiggle.hasTriggered) {
            ...
            if (Math.abs(dx) > 8) { 
                ...
                if (this.contState.squiggle.flips >= 3) {
                    ...
                    // FORCE DELETE EVENT
                    this.callbacks.onGesture({ id: 'delete', name: 'DELETE', base: 'squiggle', fingers: 1, meta: {} });
                    this.history = []; // Clear history so we don't tap after deleting
                }
            }
        }
        ...
        if (count === 2 && this.contState.squiggle2F.isTracking && !this.contState.squiggle2F.hasTriggered) {
            ...
            if (Math.abs(dx) > 8) {
                ...
                if (this.contState.squiggle2F.flips >= 3) {
                    ...
                    // FORCE CLEAR EVENT
                    this.callbacks.onGesture({ id: 'clear', name: 'CLEAR', base: 'squiggle', fingers: 2, meta: {} });
                    this.history = [];
                }
            }
        }
        ...
        if ((count === 2 || count === 3) && (now - this.contState.rotLastUpdate > 50)) {
            const p1 = pointers[0].pts.slice(-1)[0], p2 = pointers[1].pts.slice(-1)[0];
    ...
    _handleUp(e) {
        ...
        if (remaining === 0) {
            ...
            // If delete/clear happened, DO NOT analyze for taps
    ...
    _analyze() {
        ...
        inputs.forEach(s => { startSpan += Math.hypot(s.pts[0].x - sc.x, s.pts[0].y - sc.y); endSpan += Math.hypot(s.pts[s.pts.length-1].x - ec.x, s.pts[s.pts.length-1].y - ec.y); });
        ...
        let turnSum = 0;
        if (segments.length > 1) { for (let i = 0; i < segments.length - 1; i++) turnSum += this._getTurnDir(segments[i].vec, segments[i + 1].vec); }
        ...
        if (type === 'tap' && fingers === 1) {
            ...
            if (pathLen > this.config.swipeThreshold) {
                ...
                if (segments.length === 4) {
                    ...
                    if (Math.abs(a1) > 100 && Math.abs(a2) > 100 && Math.abs(a3) > 100) { type = 'double_boomerang'; shapeDetected = true; }
                }
                ...
                if (!shapeDetected) {
                    if (segments.length >= 4 && isClosed) { type = 'square'; } 
                        ...
                        const a1 = this._getAngleDiff(segments[0].vec, segments[1].vec);
                        const a2 = this._getAngleDiff(segments[1].vec, segments[2].vec);
                        if (Math.abs(a1) > 120 && Math.abs(a2) > 120) { type = 'zigzag'; }
                    } else if (segments.length === 2) {
                        const angle = this._getAngleDiff(segments[0].vec, segments[1].vec);
                        if (Math.abs(angle) > 150) { type = 'boomerang'; if (pathLen > 250) type = 'zigzag'; } else { type = 'corner'; }
                        ...
                        else if (netDist > this.config.motionTapLimit) { type = 'swipe'; meta.dir = this._getDirection(ec.x - sc.x, ec.y - sc.y); } 
                        else if (netDist > 15) { type = 'motion_tap_spatial'; meta.dir = this._getDirection(ec.x - sc.x, ec.y - sc.y); }
                    }
                } 
            } 
            // B. Spatial Taps (Small Movements)
            // FIXED: This else-if is now correctly properly reachable because the brace above matches the if(pathLen)
        ...
        if (type === 'tap') {
            this.tapStack = { active: true, count: 1, fingers: fingers, lastPos: ec, align: meta.align, timer: setTimeout(() => this._commitStack(), this.config.tapDelay) };
            return;
        }
    ...
    _emitGesture(baseType, fingers, meta) {
        ...
        if (meta && meta.dir && meta.dir !== 'none') {
            ...
            if (['square', 'triangle', 'u_shape', 'corner', 'motion_tap_corner'].includes(baseType)) { id += `_${dir}`; } 
            else if (!baseType.includes(dir)) { id += `_${dir}`; }
        } else if (['swipe', 'swipe_long', 'boomerang', 'zigzag'].includes(baseType)) { id += '_any'; }

        if (meta && meta.winding && ['square', 'triangle', 'u_shape', 'corner', 'motion_tap_corner'].includes(baseType)) id += `_${meta.winding}`;
    ...
    _segmentPath(pts) {
        ...
        for (let i = 5; i < pts.length - 5; i+=3) {
            const dx1 = pts[i].x - pts[start].x, dy1 = pts[i].y - pts[start].y;
            const dx2 = pts[i+5].x - pts[i].x, dy2 = pts[i+5].y - pts[i].y;
            const a1 = Math.atan2(dy1, dx1) * 180/Math.PI, a2 = Math.atan2(dy2, dx2) * 180/Math.PI;
            let diff = Math.abs(a1-a2); if (diff > 180) diff = 360 - diff;
            if (diff > threshold && Math.hypot(dx1,dy1) > 20) { segments.push({ dir: this._getDirection(dx1, dy1), vec: {x:dx1, y:dy1} }); start = i; }
        }
        const lastDx = pts[pts.length-1].x - pts[start].x, lastDy = pts[pts.length-1].y - pts[start].y;
    ...
    _getAngleDiff(v1, v2) { const a1 = Math.atan2(v1.y, v1.x)*180/Math.PI, a2 = Math.atan2(v2.y, v2.x)*180/Math.PI; let d = Math.abs(a1-a2); if(d>180) d=360-d; return d; }
    _getPathLen(pts) { let l=0; for(let i=1;i<pts.length;i++) l+=Math.hypot(pts[i].x-pts[i-1].x, pts[i].y-pts[i-1].y); return l; }
    _getDirection(dx, dy) {
        const ang = Math.atan2(dy, dx) * 180 / Math.PI;
        if (ang > -22.5 && ang <= 22.5) return 'right'; if (ang > 22.5 && ang <= 67.5) return 'se'; if (ang > 67.5 && ang <= 112.5) return 'down'; if (ang > 112.5 && ang <= 157.5) return 'sw'; if (ang > 157.5 || ang <= -157.5) return 'left'; if (ang > -157.5 && ang <= -112.5) return 'nw'; if (ang > -112.5 && ang <= -67.5) return 'up'; return 'ne';
    }
    _getAlignment(inputs) {
        ...
        const left = pts.reduce((a,b) => a.x < b.x ? a : b), right = pts.reduce((a,b) => a.x > b.x ? a : b);
