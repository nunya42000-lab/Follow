...
const LANG = {
    ...
    es: {
        ...
        help_stealth_detail: "El modo sigilo (1-tecla) simplifica la entrada al asignar los 12 valores primarios (1-12) a una sola pulsación de tecla. La interpretación depende del contexto y del modo (Simon/Único). Está diseñado para una entrada de alta velocidad y movimiento mínimo.", // NEW HELP DETAIL
...
export class SettingsManager {
    constructor(appSettings, callbacks, sensorEngine) {
        ...
        this.dom = {
            ...
            // Stealth Tab DOM
            hapticMorseStealth: document.getElementById('haptic-morse-toggle-stealth'), stealthPauseSelect: document.getElementById('stealth-pause-select'), blackoutToggleStealth: document.getElementById('blackout-toggle-stealth'), blackoutGesturesToggleStealth: document.getElementById('blackout-gestures-toggle-stealth'), morseMappingContainer: document.getElementById('morse-mapping-container'), gestureMappingContainer: document.getElementById('gesture-mapping-container'), gestureInputToggle: document.getElementById('gesture-input-toggle'),
...
// --- Stealth tab initialization ---
(function(){
    function buildDefaultMappings() {
        try {
            window.appSettings = window.appSettings || {};
            window.appSettings.gestureMappings = window.appSettings.gestureMappings || {};
            window.appSettings.morseMappings = window.appSettings.morseMappings || {};
            const DEFAULT_GESTURE_MAPPINGS = {
                '9key': {1:'tap',2:'doubletap',3:'longtap',4:'tap2',5:'doubletap2',6:'longtap2',7:'tap3',8:'doubletap3',9:'longtap3'},
                '12key': {1:'swipeleft',2:'swipedown',3:'swipeup',4:'swiperight',5:'swipeleft2',6:'swipedown2',7:'swipeup2',8:'swiperight2',9:'swipeleft3',10:'swipedown3',11:'swipeup3',12:'swiperight3'},
                'piano': {1:'swipeleft2',2:'swipeup2left',3:'swipeup2',4:'swipeupright2',5:'swiperight2','C':'swipeup2left','D':'swipeleft2','E':'swipeup2right','F':'swipedown2','G':'swipedownright2','A':'swiperight2','B':'swipeupright2'}
            };
            const DEFAULT_MORSE = {
            ...
            for(const k in DEFAULT_GESTURE_MAPPINGS) { if(!window.appSettings.gestureMappings[k]) window.appSettings.gestureMappings[k] = DEFAULT_GESTURE_MAPPINGS[k]; }
            for(const k in DEFAULT_MORSE) { if(!window.appSettings.morseMappings[k]) window.appSettings.morseMappings[k] = DEFAULT_MORSE[k]; }
        } catch(e){ console.warn('buildDefaultMappings error', e); }
    }
    
    function populateMappingContainers() {
        try {
            const morseC = document.getElementById('morse-mapping-container');
            const gestureC = document.getElementById('gesture-mapping-container');
            if(!morseC || !gestureC) return;
            morseC.innerHTML = ''; gestureC.innerHTML = '';
            const types = ['9key','12key','piano'];
            const gestureOptions = [
                {group:'Taps', items:[{v:'tap',t:'Tap'},{v:'doubletap',t:'Double Tap'},{v:'longtap',t:'Long Tap'}]},
                {group:'Two-Finger Taps', items:[{v:'tap2',t:'2-Finger Tap'},{v:'doubletap2',t:'2-Finger Double Tap'},{v:'longtap2',t:'2-Finger Long Tap'}]},
                {group:'Three-Finger Taps', items:[{v:'tap3',t:'3-Finger Tap'},{v:'doubletap3',t:'3-Finger Double Tap'},{v:'longtap3',t:'3-Finger Long Tap'}]},
                {group:'Swipes (1 finger)', items:[{v:'swipeleft',t:'Swipe Left'},{v:'swiperight',t:'Swipe Right'},{v:'swipeup',t:'Swipe Up'},{v:'swipedown',t:'Swipe Down'},{v:'swipeupleft',t:'Swipe Up-Left'},{v:'swipeupright',t:'Swipe Up-Right'},{v:'swipedownleft',t:'Swipe Down-Left'},{v:'swipedownright',t:'Swipe Down-Right'}]},
                {group:'Swipes (2 fingers)', items:[{v:'swipeleft2',t:'Swipe Left (2)'},{v:'swiperight2',t:'Swipe Right (2)'},{v:'swipeup2',t:'Swipe Up (2)'},{v:'swipedown2',t:'Swipe Down (2)'},{v:'swipeupleft2',t:'Swipe Up-Left (2)'},{v:'swipeupright2',t:'Swipe Up-Right (2)'},{v:'swipedownleft2',t:'Swipe Down-Left (2)'},{v:'swipedownright2',t:'Swipe Down-Right (2)'}]},
                {group:'Swipes (3 fingers)', items:[{v:'swipeleft3',t:'Swipe Left (3)'},{v:'swiperight3',t:'Swipe Right (3)'},{v:'swipeup3',t:'Swipe Up (3)'},{v:'swipedown3',t:'Swipe Down (3)'},{v:'swipeupleft3',t:'Swipe Up-Left (3)'},{v:'swipeupright3',t:'Swipe Up-Right (3)'},{v:'swipedownleft3',t:'Swipe Down-Left (3)'},{v:'swipedownright3',t:'Swipe Down-Right (3)'}]}
            ];
            const morseOptions = [{v:'.',t:'.'},{v:'..',t:'..'},{v:'...',t:'...'},{v:'-',t:'-'},{v:'-.',t:'-.'},{v:'-..',t:'-..'},{v:'--',t:'--'},{v:'--.',t:'--.'},{v:'---',t:'---'},{v:'-.-',t:'-.-'},{v:'.-',t:'.-'},{v:'.-.',t:'.-.'}];
            types.forEach(type => {
                const box = document.createElement('div');
                box.className = 'p-2 rounded border border-custom settings-input mb-2';
                const title = document.createElement('div'); title.className='font-bold mb-2'; title.textContent = type.toUpperCase() + ' Mapping';
                box.appendChild(title);
                const list = document.createElement('div'); list.className='space-y-1';
                const count = (type==='9key')?9: (type==='12key')?12:12;
                const pianoLabels = ['1','2','3','4','5','C','D','E','F','G','A','B'];
                for(let i=1;i<=count;i++){
                    const row = document.createElement('div'); row.className='flex items-center justify-between';
                    const lbl = document.createElement('div'); lbl.style.width='20%';
                    if(type==='piano') lbl.textContent = pianoLabels[i-1];
                    else lbl.textContent = type + ' - ' + i;
                    const morseSel = document.createElement('select'); morseSel.className='settings-input p-1 rounded w-1/3 text-sm'; morseSel.dataset.type = type; morseSel.dataset.index = i;
                    morseOptions.forEach(opt=>{ const o=document.createElement('option'); o.value=opt.v; o.textContent=opt.t; morseSel.appendChild(o); });
                    const gestSel = document.createElement('select'); gestSel.className='settings-input p-1 rounded w-1/3 text-sm'; gestSel.dataset.type = type; gestSel.dataset.index = i;
                    gestureOptions.forEach(group=>{
                        const og = document.createElement('optgroup'); og.label = group.group;
                        group.items.forEach(it=>{ const o=document.createElement('option'); o.value = it.v; o.textContent = it.t; og.appendChild(o); });
                        gestSel.appendChild(og);
                    });
                    if(window.appSettings && window.appSettings.morseMappings && window.appSettings.morseMappings[type] && window.appSettings.morseMappings[type][i]) morseSel.value = window.appSettings.morseMappings[type][i];
                    if(window.appSettings && window.appSettings.gestureMappings && window.appSettings.gestureMappings[type] && window.appSettings.gestureMappings[type][i]) gestSel.value = window.appSettings.gestureMappings[type][i];
                    morseSel.onchange = function(){ window.appSettings.morseMappings = window.appSettings.morseMappings || {}; window.appSettings.morseMappings[type] = window.appSettings.morseMappings[type] || {}; window.appSettings.morseMappings[type][i] = morseSel.value; if(window.settingsManager && settingsManager.callbacks && settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
                    gestSel.onchange = function(){ window.appSettings.gestureMappings = window.appSettings.gestureMappings || {}; window.appSettings.gestureMappings[type] = window.appSettings.gestureMappings[type] || {}; window.appSettings.gestureMappings[type][i] = gestSel.value; if(window.settingsManager && settingsManager.callbacks && settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
                    row.appendChild(lbl); row.appendChild(morseSel); row.appendChild(gestSel);
                    list.appendChild(row);
                }
                box.appendChild(list);
                morseC.appendChild(box);
                const guide = document.createElement('div'); guide.className='p-2 rounded border border-custom settings-input mb-2'; guide.innerHTML = '<div class=\"font-bold mb-2\">Gesture Reference</div><div class=\"text-sm text-muted\">Configure gestures corresponding to each input above.</div>'; gestureC.appendChild(guide);
            });
        } catch(e){ console.warn('populateMappingContainers error', e); }
    }
    
    function initStealthTab() {
        buildDefaultMappings();
        populateMappingContainers();
        const haptic = document.getElementById('haptic-morse-toggle-stealth');
        const pause = document.getElementById('stealth-pause-select');
        const bOff = document.getElementById('blackout-toggle-stealth');
        const bg = document.getElementById('blackout-gestures-toggle-stealth');
        if(haptic) haptic.checked = !!(window.appSettings && window.appSettings.isHapticMorseEnabled);
        if(pause) pause.value = (window.appSettings && typeof window.appSettings.stealthPause !== 'undefined')? window.appSettings.stealthPause : '0.2';
        if(bOff) bOff.checked = !!(window.appSettings && window.appSettings.isBlackoutFeatureEnabled);
        if(bg) bg.checked = !!(window.appSettings && window.appSettings.isBlackoutGesturesEnabled);
        if(pause) pause.onchange = function(){ if(window.appSettings) window.appSettings.stealthPause = parseFloat(pause.value); if(window.settingsManager && settingsManager.callbacks && settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
        if(haptic) haptic.onchange = function(){ if(window.appSettings) window.appSettings.isHapticMorseEnabled = haptic.checked; if(window.settingsManager && settingsManager.callbacks && settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
        if(bOff) bOff.onchange = function(){ if(window.appSettings) window.appSettings.isBlackoutFeatureEnabled = bOff.checked; if(window.settingsManager && settingsManager.callbacks && settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
        if(bg) bg.onchange = function(){ if(window.appSettings) window.appSettings.isBlackoutGesturesEnabled = bg.checked; if(window.settingsManager && settingsManager.callbacks and settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
    }
    if(document.readyState==='complete' || document.readyState==='interactive') { setTimeout(initStealthTab,50); }
    else document.addEventListener('DOMContentLoaded', initStealthTab);
})();
