...
(function() {
    ...
    // Robust initialization that waits for the DOM
    async function initApp() {
        loadState(); 
        assignDomElements();
        
        // Safety check: If this is null, the HTML didn't load before the script ran
        if (!DOM.sequenceContainer) {
            console.warn("DOM not ready yet, retrying...");
        ...
        // Open Setup if needed
        if (appSettings.showWelcomeScreen) {
            // Small delay to ensure CSS transitions invoke smoothly
            setTimeout(openGameSetupModal, 100); 
        }
        
        // Wake up audio engine
        if (getCurrentProfileSettings().isAudioEnabled) speak(" "); 
        
        console.log("Follow Me App Initialized Successfully");
    }
    ...
    function initializeListeners() {
        // --- MASTER SWITCHES (The Camera/Mic Icons on the Pad) ---
        // These control whether the Camera/Mic actually inputs numbers
        document.body.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#camera-master-btn')) {
                ...
                showToast(`Camera Input: ${Camera.cameraState.isCameraMasterOn ? 'ON' : 'OFF'}`);
            }
            // Mic Master Switch
            if (e.target.closest('#mic-master-btn')) {
                isMicMasterOn = !isMicMasterOn;
                UI.updateMainUIControlsVisibility(Camera.cameraState.isCameraMasterOn, isMicMasterOn);
                showToast(`Mic Input: ${isMicMasterOn ? 'ON' : 'OFF'}`);
            }
        });
        ...
        document.addEventListener('click', (event) => {
            ...
            if (action === 'open-help') { UI.closeModal(); openHelpModal(); return; }
            ...
            // 5. Game Inputs (Numbers, Demo)
            const currentInput = getCurrentProfileSettings().currentInput;
            
            // Only allow inputs from the ACTIVE pad (prevent hidden buttons from firing)
        ...
        if (DOM.quickOpenHelpBtn) DOM.quickOpenHelpBtn.addEventListener('click', () => { closeGameSetupModal(); openHelpModal(); });
            ...
            if (element) {
                element.addEventListener(eventType, (e) => {
                    ...
                    if (element === DOM.autoInputSlider) { 
                        ...
                        // Update Master Button Visibility immediately
                        UI.updateMainUIControlsVisibility(Camera.cameraState.isCameraMasterOn, isMicMasterOn);
                    }
