...
(function() {
    ...
    // --- (Updated) Premade profiles will inherit new camera settings
    ...
    function closeCameraModal() {
        if (!cameraModal) return;
        stopCameraStream(); // Always stop camera on close
    ...
    function stopCameraStream() {
        ...
        if (stopDetectionBtn) stopDetectionBtn.style.display = 'none';
        isDetecting = false;
        // TODO: Cancel detection loop if running
    }
...
Let's start with Round 1.`;
    ...
    function initializeListeners() {
        ...
        document.addEventListener('click', (event) => {
            ...
            if (action === 'open-camera') { closeSettingsModal(); openCameraModal(); return; } // <-- NEW
            if (action === 'copy-link') {
        ...
        const addProfileSettingListener = (element, eventType, settingKey, valueType = 'value') => {
            if (element) {
                element.addEventListener(eventType, (e) => {
                    ...
                    if (element === flashSensitivitySlider) updateFlashSensitivityDisplay(value); // <-- NEW
        ...
        // --- (NEW) Grid Drag/Resize Listeners ---
        if (calibrationGridContainer) {
            ...
            calibrationGridContainer.addEventListener('mouseup', saveGridConfig);
            calibrationGridContainer.addEventListener('touchend', saveGridConfig);
        }
