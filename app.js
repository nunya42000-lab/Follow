...
// Practice start visibility helper
(function(){
    function updatePracticeStartVisibility() {
        try {
            ...
            const settingsModal = document.getElementById('settings-modal');
            if(!startBtn) return;
            if(window.appSettings && window.appSettings.isPracticeModeEnabled && (!settingsModal || settingsModal.classList.contains('pointer-events-none'))) startBtn.classList.remove('hidden');
            else startBtn.classList.add('hidden');
        } catch(e){}
    }
    const origRender = window.renderUI;
    window.renderUI = function(){ try{ if(origRender) origRender(); }catch(e){} updatePracticeStartVisibility(); }
})();