...
export class SettingsManager {
    constructor(appSettings, callbacks, sensorEngine) {
        ...
        // Inject DOM elements if missing (Safety check)
    ...
    injectLongPressToggle() {
        ...
        if (speedDeleteInput && speedDeleteInput.parentElement) {
            ...
            if(generalTab) generalTab.querySelector('.space-y-3').appendChild(div);
        }
    }
    ...
    injectBlackoutGesturesToggle() {
        ...
        if (blackoutToggleInput && blackoutToggleInput.parentElement) {
            blackoutToggleInput.parentElement.parentElement.insertBefore(div, blackoutToggleInput.parentElement.nextSibling);
        }
    }
    ...
    injectGestureInputToggle() {
        ...
        if (stealthRow && stealthRow.parentElement) {
            stealthRow.parentElement.parentElement.insertBefore(div, stealthRow.parentElement.nextSibling);
        }
    }
...
initListeners() {
    ...
    if (this.dom.timerToggle) {
        this.dom.timerToggle.onchange = (e) => {
            ...
            // If both are off, hide the header entirely (opacity 0)
...
populateMappingUI() {
    ...
    const makeRow = (labelText, keyName, mappingId) => {
        ...
        // Changed from space-x-2 to gap-2 for better horizontal spacing
        wrapper.className = "flex items-center gap-2 mapping-row"; 
        
        const lbl = document.createElement('div');
        lbl.className = "text-sm font-semibold w-8 text-center text-muted-custom";
        lbl.textContent = labelText;
        
        const gestureSelect = document.createElement('select');
        // settings-input ensures dark theme. 
        gestureSelect.className = "settings-input p-2 rounded flex-grow text-xs font-bold";
        ...
        morseSelect.className = "settings-input p-2 rounded w-24 text-xs font-bold font-mono";
    ...
    // Use gap-2 to add slight breathing room between rows
    if(container9) {
        container9.innerHTML = '';
        container9.className = "grid grid-cols-1 gap-2"; 
        for(let i=1;i<=9;i++){
            const key = 'k9_' + i;
    ...
    if(container12) {
        ...
        container12.className = "grid grid-cols-1 gap-2";
        for(let i=1;i<=12;i++){
            const key = 'k12_' + i;
    ...
    if(containerPiano) {
        ...
        containerPiano.className = "grid grid-cols-1 gap-2";
