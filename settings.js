...
export class SettingsManager {
    // New stealth tab DOM refs will be created dynamically
...
// --- Injected Timer/Counter setting DOM hooks ---
(function(){
    // Wait for DOM elements to be present (settings modal may be built earlier)
    function initTimerCounterSettings() {
        try {
            const timerToggle = document.getElementById('timer-toggle');
            const counterToggle = document.getElementById('counter-toggle');
            // If the toggles don't exist in DOM, create and inject them into General tab near other toggles
            const generalTab = document.querySelector('#tab-general .tab-content') || document.querySelector('#tab-general') || document.querySelector('.tab-content[data-tab="general"]');
            if(!timerToggle && generalTab) {
                const divTimer = document.createElement('div');
                divTimer.className = 'flex justify-between items-center p-3 rounded-lg settings-input';
                divTimer.innerHTML = `<span class="font-bold text-sm">Show Timer Button</span><input type="checkbox" id="timer-toggle" class="h-5 w-5 accent-indigo-500">`;
                generalTab.appendChild(divTimer);
            }
            if(!counterToggle && generalTab) {
                const divCount = document.createElement('div');
                divCount.className = 'flex justify-between items-center p-3 rounded-lg settings-input';
                divCount.innerHTML = `<span class="font-bold text-sm">Show Counter Button</span><input type="checkbox" id="counter-toggle" class="h-5 w-5 accent-indigo-500">`;
                generalTab.appendChild(divCount);
            }
            // Wire to appSettings if the global SettingsManager exists
            if(window.settingsManager && window.appSettings) {
                ...
                if(t) {
                    t.checked = !!app.showTimerBtn;
                    t.onchange = () => { app.showTimerBtn = t.checked; if(mgr.callbacks && mgr.callbacks.onSave) mgr.callbacks.onSave(); if(mgr.callbacks && mgr.callbacks.onUpdate) mgr.callbacks.onUpdate(); if(window.renderUI) renderUI(); };
                }
                if(c) {
                    c.checked = !!app.showCounterBtn;
                    c.onchange = () => { app.showCounterBtn = c.checked; if(mgr.callbacks && mgr.callbacks.onSave) mgr.callbacks.onSave(); if(mgr.callbacks && mgr.callbacks.onUpdate) mgr.callbacks.onUpdate(); if(window.renderUI) renderUI(); };
                }
            }
        } catch(e) {
            console.warn('Timer/Counter settings init failed', e);
        }
    }
    if(document.readyState === 'complete' || document.readyState === 'interactive') setTimeout(initTimerCounterSettings, 50);
    else document.addEventListener('DOMContentLoaded', initTimerCounterSettings);
})();


// --- Stealth tab and gesture/morse mapping helpers ---
(function(){
    function ensureStealthUI() {
        try {
            const morseContainer = document.getElementById('morse-mapping-container');
            const gestureContainer = document.getElementById('gesture-mapping-container');
            if(!morseContainer || !gestureContainer) return;
            // Build default mapping UI for three input types
            const inputTypes = ['9key','12key','piano'];
            inputTypes.forEach(type => {
                const box = document.createElement('div');
                box.className = 'p-3 rounded-lg border border-custom settings-input mb-2';
                const title = document.createElement('div');
                title.className = 'font-bold mb-2';
                title.textContent = type.toUpperCase() + ' Mapping';
                box.appendChild(title);
                const list = document.createElement('div');
                list.className = 'space-y-1';

                // create 12 or 9 or 7 rows depending on type
                const count = (type==='9key')?9: (type==='12key')?12:7;
                for(let i=1;i<=count;i++){
                    const row = document.createElement('div');
                    row.className = 'flex items-center justify-between';
                    const lbl = document.createElement('div');
                    lbl.textContent = type + ' - ' + i;
                    const inp = document.createElement('input');
                    inp.type = 'text';
                    inp.className = 'settings-input p-1 rounded w-1/2 text-sm';
                    inp.dataset.inputType = type;
                    inp.dataset.index = i;
                    // load from appSettings if present
                    if(window.appSettings && window.appSettings.gestureMappings && window.appSettings.gestureMappings[type] && window.appSettings.gestureMappings[type][i]) {
                        inp.value = window.appSettings.gestureMappings[type][i];
                    } else {
                        inp.value = '';
                    }
                    inp.onchange = function(){
                        ...
                        window.appSettings.gestureMappings[type] = window.appSettings.gestureMappings[type] || {};
                        window.appSettings.gestureMappings[type][i] = inp.value;
                        if(window.settingsManager && window.settingsManager.callbacks && window.settingsManager.callbacks.onSave) settingsManager.callbacks.onSave();
                    };
                    row.appendChild(lbl);
                    row.appendChild(inp);
                ...
                const c = (type==='piano')?gestureContainer:morseContainer;
                c.appendChild(box);
            });
        } catch(e){ console.warn('Stealth UI build failed', e); }
    }
    function initStealthUIWhenReady(){ if(document.readyState==='complete' || document.readyState==='interactive') setTimeout(ensureStealthUI,100); else document.addEventListener('DOMContentLoaded', ensureStealthUI); }
    initStealthUIWhenReady();
})();
