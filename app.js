...
/* ===== HEADER INTEGRATION: timer, counter, mic/cam in the header =====
...
(function () {
    ...
    whenReady(() => {
        function waitForAppEnv(cb, attempts = 0) {
            if (typeof window.appSettings !== 'undefined' && typeof window.saveState === 'function') return cb();
            if (attempts > 40) return cb();
            setTimeout(() => waitForAppEnv(cb, attempts + 1), 150);
        }

        waitForAppEnv(() => {
            function toggleHeaderBtn(id, show) {
            ...
            function updateHeaderVisibility() {
                ...
                if (!bar) return;
                const showTimer = !!(window.appSettings && window.appSettings.showTimerBtn);
                const showCounter = !!(window.appSettings && window.appSettings.showCounterBtn);
                const showMic = !!(window.appSettings && window.appSettings.showMicBtn);
                const showCam = !!(window.appSettings && window.appSettings.showCamBtn);
                ...
                toggleHeaderBtn('header-timer-btn', showTimer);
                toggleHeaderBtn('header-counter-btn', showCounter);
                toggleHeaderBtn('header-mic-btn', showMic);
                toggleHeaderBtn('header-cam-btn', showCam);
            }

            let headerTimer = { running: false, startTs: 0, accumMs: 0, interval: null };
            function headerTimerFormat(ms) {
                const s = Math.floor(ms / 1000);
                const mm = String(Math.floor(s / 60)).padStart(2, '0');
                const ss = String(s % 60).padStart(2, '0');
                return `${mm}:${ss}`;
            }
            function headerTimerTick() {
                const btn = document.getElementById('header-timer-btn');
                if (!btn) return;
                const nowMs = Date.now();
                const elapsed = headerTimer.accumMs + (headerTimer.running ? (nowMs - headerTimer.startTs) : 0);
                btn.textContent = headerTimerFormat(elapsed);
            }
            function startHeaderTimer() {
                if (headerTimer.running) return;
                headerTimer.startTs = Date.now();
                headerTimer.running = true;
                headerTimer.interval = setInterval(headerTimerTick, 250);
                headerTimerTick();
            }
            function stopHeaderTimer() {
                if (!headerTimer.running) return;
                const now = Date.now();
                headerTimer.accumMs += (now - headerTimer.startTs);
            ...
            function resetHeaderTimer() {
                headerTimer.running = false;
                headerTimer.startTs = 0;
                headerTimer.accumMs = 0;
                if (headerTimer.interval) { clearInterval(headerTimer.interval); headerTimer.interval = null; }
                headerTimerTick();
            }

            let headerCounter = { value: 0 };
            function headerCounterSet(v) {
                headerCounter.value = v;
                const btn = document.getElementById('header-counter-btn');
                if (btn) btn.textContent = String(headerCounter.value);
                if (window.appSettings) {
                    window.appSettings.headerCounterValue = headerCounter.value;
            ...
            function initHeaderControls() {
                ...
                if (tBtn) {
                    ...
                    tBtn.addEventListener('pointerdown', (e) => {
                        e.preventDefault();
                        tTimer = setTimeout(() => { resetHeaderTimer(); }, 600);
                    });
                    tBtn.addEventListener('pointerup', (e) => {
                        e.preventDefault();
                        if (tTimer) { clearTimeout(tTimer); tTimer = null; startStopToggleHeaderTimer(); }
                    });
                    tBtn.addEventListener('pointercancel', () => { if (tTimer) { clearTimeout(tTimer); tTimer = null; } });
                }

                function startStopToggleHeaderTimer() {
                    if (headerTimer.running) stopHeaderTimer();
                ...
                if (cBtn) {
                    ...
                    cBtn.addEventListener('pointerdown', (e) => {
                        e.preventDefault();
                        cTimer = setTimeout(() => { headerCounterSet(0); }, 600);
                    });
                    cBtn.addEventListener('pointerup', (e) => {
                        e.preventDefault();
                        if (cTimer) { clearTimeout(cTimer); cTimer = null; headerCounterSet(headerCounter.value + 1); }
                    });
                    cBtn.addEventListener('pointercancel', () => { if (cTimer) { clearTimeout(cTimer); cTimer = null; } });
                }

                if (micBtn) {
                    micBtn.addEventListener('click', async (e) => {
                        e.preventDefault();
                        if (window.appSettings) window.appSettings.showMicBtn = true;
                        if (window.modules && window.modules.sensor && typeof window.modules.sensor.toggleAudio === 'function') {
                            try {
                                const current = window.modules.sensor.mode?.audio;
                                await window.modules.sensor.toggleAudio(!current);
                                if (window.appSettings) window.appSettings.showMicBtn = !!(!current);
                            } catch (err) { console.warn('toggleAudio failed', err); }
                        }
                        if (typeof window.saveState === 'function') window.saveState();
                        updateHeaderVisibility();
                        if (typeof window.showToast === 'function') showToast('Mic toggled');
                    });
                }
                if (camBtn) {
                    camBtn.addEventListener('click', async (e) => {
                        e.preventDefault();
                        if (window.modules && window.modules.sensor && typeof window.modules.sensor.toggleCamera === 'function') {
                            try {
                                const current = window.modules.sensor.mode?.camera;
                                await window.modules.sensor.toggleCamera(!current);
                                if (window.appSettings) window.appSettings.showCamBtn = !!(!current);
                            } catch (err) { console.warn('toggleCamera failed', err); }
                        }
                        if (typeof window.saveState === 'function') window.saveState();
                        updateHeaderVisibility();
                        if (typeof window.showToast === 'function') showToast('Camera toggled');
                    });
                }
            }

            function onSettingsChanged_updateHeader() {
                if (typeof window.appSettings === 'undefined') window.appSettings = {};
                if (typeof window.appSettings.showTimerBtn === 'undefined') window.appSettings.showTimerBtn = false;
                if (typeof window.appSettings.showCounterBtn === 'undefined') window.appSettings.showCounterBtn = false;
                if (typeof window.appSettings.headerCounterValue !== 'undefined') headerCounterSet(parseInt(window.appSettings.headerCounterValue) || 0);
                else headerCounterSet(headerCounter.value || 0);
                if (!window.appSettings.showTimerBtn) resetHeaderTimer();
                updateHeaderVisibility();
            }

            function initHeaderIntegration() {
                if (typeof window.appSettings !== 'undefined' && typeof window.appSettings.headerCounterValue !== 'undefined') {
                    headerCounterSet(parseInt(window.appSettings.headerCounterValue) || 0);
                } else {
                    headerCounterSet(0);
                }

                initHeaderControls();
                onSettingsChanged_updateHeader();

                document.querySelectorAll('.auto-input-btn').forEach(el => el.classList.add('hidden'));

                if (typeof window.onSettingsChanged_updateHeader === 'undefined') {
                    window.onSettingsChanged_updateHeader = onSettingsChanged_updateHeader;
                }
            }

            setTimeout(() => {
                initHeaderIntegration();
            }, 350);
        });
    });
})();
