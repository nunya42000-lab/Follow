...
export class GestureEngine {
    ...
    _handleMove(e) {
        ...
        if ((count === 2 || count === 3) && (now - this.contState.rotLastUpdate > 50)) {
            ...
            if (Math.abs(this.contState.rotAccumulator) > 15) {
                this.callbacks.onContinuous({
                    type: 'twist',
                    fingers: count,
                    value: this.contState.rotAccumulator > 0 ? 1 : -1
                });
                this.contState.rotAccumulator = 0;
        ...
        if (count === 2 && this.contState.pinchStartDist > 0) {
            ...
            if (Math.abs(dist - this.contState.pinchStartDist) > 20) {
                this.callbacks.onContinuous({
                    type: 'pinch',
                    scale: dist / this.contState.pinchStartDist
                });
            }
        }
    }
    ...
    _analyze() {
        ...
        inputs.forEach(s => {
            sc.x += s.pts[0].x;
            sc.y += s.pts[0].y;
            ec.x += s.pts[s.pts.length - 1].x;
            ec.y += s.pts[s.pts.length - 1].y;
        });
        sc.x /= inputs.length;
        sc.y /= inputs.length;
        ec.x /= inputs.length;
        ...
        if (fingers === 2 && pathLen > 40 && netDist > 40) {
            ...
            if (Math.abs(endSpan - startSpan) > 30) {
                ...
                if (endSpan < startSpan * 0.7) {
                    type = 'pinch_swipe';
                    meta.dir = dir;
                } else if (endSpan > startSpan * 1.3) {
                    type = 'expand_swipe';
                    meta.dir = dir;
                }
        ...
        if (this.tapStack.active) {
            ...
            if (type === 'tap' && fingers === this.tapStack.fingers) {
                ...
                if (seqDist > 50 && fingers === 1) {
                    ...
                    this._emitGesture('motion_tap', fingers, {
                        subMode: 'spatial',
                        dir: dir
                    });
                    this._clearStack();
            ...
            if (type !== 'tap' && fingers === 1 && this.tapStack.fingers === 1) {
                this._emitGesture('motion_tap', 1, {
                    subMode: type,
                    dir: meta.dir,
    ...
    _commitStack() {
        ...
        if (count > 0) {
            ...
            if (maxDist > 50 && fingers === 1 && count >= 2) {
                ...
                if (count === 3) {
                    const v1 = {
                        x: posHistory[1].x - posHistory[0].x,
                        y: posHistory[1].y - posHistory[0].y
                    };
                    const v2 = {
                        x: posHistory[2].x - posHistory[1].x,
                        y: posHistory[2].y - posHistory[1].y
                    };
                    ...
                    this._emitGesture('triple_tap', fingers, {
                        subMode: subMode,
                        dir: finalDir
                    });
                }
                ...
                this._emitGesture(type, fingers, {
                    align: align
                });
            }
            this._clearStack();
    ...
    _emitGesture(baseType, fingers, meta, overrideName = null) {
        ...
        const tryFallback = (candidate) => {
            if (this.allowedGestures && this.allowedGestures.has(candidate)) {
                finalId = candidate;
                return true;
            }
            return false;
    ...
    _cleanSegments(segments) {
        return segments.filter(s => Math.hypot(s.vec.x, s.vec.y) > 15);
    }
    _mergeSegments(segments) {
        ...
        for (let i = 1; i < segments.length; i++) {
            ...
            if (Math.abs(this._getAngleDiff(current.vec, next.vec)) < 45) {
                current.vec.x += next.vec.x;
                current.vec.y += next.vec.y;
    ...
    _segmentPath(pts) {
        ...
        for (let i = 2; i < pts.length - 2; i++) {
            ...
            const dx2 = pts[nextIdx].x - pts[i].x;
            const dy2 = pts[nextIdx].y - pts[i].y;
        ...
        if (Math.hypot(lastDx, lastDy) > 10) segments.push({
