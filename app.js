...
let appSettings = {
    ...
    voicePresets: {},
    customGestures: [], 
    gestureMappings: {}, // Added to ensure mapping lookups work
    generalActions: [
        {gesture: 'shake', action: 'act_toggle_input'},
        {gesture: 'cam_cover', action: 'act_toggle_blackout'},
        {gesture: 'trigger_shake', action: 'act_toggle_input'}, // Compatibility
        {gesture: 'trigger_cam_cover', action: 'act_toggle_blackout'} // Compatibility
    ], 
...
async function initApp() {
    ...
    try {
        ...
        if(practiceStartBtn) {
            practiceStartBtn.onclick = () => { vibrate(); startPracticeRound(); };
        }

        // Show Welcome Modal
        if(appSettings.showWelcomeScreen) {
            setTimeout(() => {
                if(modules.settings) modules.settings.openSetup();
            }, 500);
        }
        ...
        alert("App crashed: " + e.message + ". Try clearing cache.");
    }
}
...
function renderUI() {
    ...
    // Toggle Input Pads
    ['pad-key9', 'pad-key12', 'pad-piano'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.style.display = 'none';
    });
    
    if(ps.currentInput === CONFIG.INPUTS.KEY9) document.getElementById('pad-key9').style.display = 'block';
    if(ps.currentInput === CONFIG.INPUTS.KEY12) document.getElementById('pad-key12').style.display = 'block';
    if(ps.currentInput === CONFIG.INPUTS.PIANO) document.getElementById('pad-piano').style.display = 'block';
...
function handleInput(val) {
    ...
    // 2. Standard Mode Initialization
    ...
    // 4. Unique Mode Logic
        ...
        // Auto-add to Master Sequence if user catches up (Keep Pace Logic)
        while (state.sequence.length < state.userSequence.length) {
            addToSequence();
        }
        
        // Ensure master sequence has a value for current position if missing
...
function initGesturePad() {
    // This handles both the full screen pad and gesture logic
    const pad = document.getElementById('gesture-pad');
    if(!pad && !appSettings.isGestureInputEnabled) return; 
    
    // Attach listener to window if fullscreen mode, otherwise just pad
    const target = appSettings.isGestureInputEnabled ? window : (pad || window);
    ...
    const start = (e) => {
        // Only prevent default if we are in gesture mode to avoid blocking normal UI interactions
        if(appSettings.isGestureInputEnabled) e.preventDefault();
        
        isTouchActive = true;
        clearTimeout(timer);
        const t = e.touches ? e.touches[0] : e;
        // Use client coordinates for simplicity in full screen
        currentStroke = [{x: t.clientX, y: t.clientY}];
    };
    ...
    const move = (e) => {
        ...
        if(appSettings.isGestureInputEnabled) e.preventDefault();
        const t = e.touches ? e.touches[0] : e;
        currentStroke.push({x: t.clientX, y: t.clientY});
    };
    ...
    const end = (e) => {
        ...
        // Wait briefly for multi-stroke gestures
        timer = setTimeout(() => {
            const fingers = e.changedTouches ? e.changedTouches.length : 1; 
            processGestures(strokes, fingers); 
            strokes = [];
        }, 400);
    };
...
function processGestures(strokes, fingers) {
    ...
    // 1. Check for Custom Match first
    ...
    if(strokes.length === 1) {
        ...
        let action = '';
        // Approximate finger count based on heuristic or event data (simplified here)
...
function resolveGesture(code) {
    ...
    if(modules.settings) {
        ...
        // Reverse lookup the map to find which Key corresponds to this gesture
...
window.appState = state;
 appSettings.isPracticeModeEnabled ? state.practiceSequence : state.sequence;
