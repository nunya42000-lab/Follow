// gestures.js
// Version: v71 - Strict ID Matching & Directional Multi-Taps
...
export class GestureEngine {
    constructor(targetElement, config, callbacks) {
        ...
        this.config = Object.assign({
            tapDelay: 300,        
            swipeThreshold: 30,   
            spatialThreshold: 10, // Micro-swipes (Spatial Taps)
            tapPrecision: 30,     
            debug: false
        }, config || {});
        ...
        this.contState = {
            rotStartAngle: 0, rotAccumulator: 0, rotLastUpdate: 0,
            pinchStartDist: 0,
    ...
    _handleMove(e) {
        ...
        if (this.contState.squiggle.isTracking || this.contState.squiggle2F.isTracking) { if (e.cancelable) e.preventDefault(); }
        ...
        if (count === 1 && this.contState.squiggle.isTracking && !this.contState.squiggle.hasTriggered) {
            ...
            if (Math.abs(dx) > 8) { 
                ...
                if (this.contState.squiggle.flips >= 3) {
                    ...
                    this._emitGesture('delete', 1, {}, 'DELETE'); // Fixed Name
                }
            }
        }
        ...
        if (count === 2 && this.contState.squiggle2F.isTracking && !this.contState.squiggle2F.hasTriggered) {
            ...
            if (Math.abs(dx) > 8) {
                ...
                if (this.contState.squiggle2F.flips >= 3) {
                    ...
                    this._emitGesture('clear', 2, {}, 'CLEAR'); // Fixed Name
                }
            }
        }
    ...
    _analyze() {
        ...
        // 2. One Finger Shapes/Swipes/Spatial
        if (fingers === 1) {
            if (pathLen > this.config.swipeThreshold) {
                ...
                if (!shapeDetected) {
                    if (segments.length >= 4 && isClosed) type = 'square';
                        ...
                        if (netDist > 150) type = 'swipe_long';
                        else if (netDist > this.config.swipeThreshold) type = 'swipe';
                        else if (netDist > this.config.spatialThreshold) type = 'spatial_tap';
                    }
                }
            ...
            // Motion Taps (Shapes drawn small)
    ...
    _handleTapStack(pos, fingers, meta) {
        ...
        this.tapStack = { active: true, count: 1, fingers: fingers, history: [pos], align: meta.align, timer: setTimeout(() => this._commitStack(), this.config.tapDelay) };
    }
    ...
    _commitStack() {
        ...
        // 2. Double Tap (Directional)
        if (count === 2) {
            const p1 = history[0], p2 = history[1];
        ...
        // 3. Triple Tap (Directional)
        if (count === 3) {
            const p1 = history[0], p2 = history[1], p3 = history[2];
            const v1 = { x: p2.x - p1.x, y: p2.y - p1.y };
            const v2 = { x: p3.x - p2.x, y: p3.y - p2.y };
            const d1 = Math.hypot(v1.x, v1.y), d2 = Math.hypot(v2.x, v2.y);
            
            let type = 'triple_tap';
            let dir = null, winding = null;
    ...
    _emitGesture(baseType, fingers, meta, overrideName = null) {
        ...
        if (meta && meta.dir && meta.dir !== 'none') {
            ...
            // Force direction suffix for specific types
            ...
            // Force _any suffix for specific types if NO direction found
        ...
        // Winding Suffixes (cw/ccw)
        if (meta && meta.winding) id += `_${meta.winding}`;

        // Alignment Suffixes (Vertical/Horizontal)
        if (meta && meta.align) {
        ...
        // Multi-Finger Suffix Safety
        if (fingers > 1 && !id.includes(`${fingers}f`) && !baseType.includes(`${fingers}f`)) {
            id += `_${fingers}f`;
        }

        // --- NAME CLEANUP (Requests from User) ---
        // Ensure "motion_tap_swipe" naming convention
        if (id.startsWith('motion_tap_swipe') && !id.includes('_long')) { /* good */ }
        
        // Final ID is the name
    ...
    _getDirection(dx, dy) {
        ...
        if (ang > -22.5 && ang <= 22.5) return 'right'; if (ang > 22.5 && ang <= 67.5) return 'se';
        if (ang > 67.5 && ang <= 112.5) return 'down'; if (ang > 112.5 && ang <= 157.5) return 'sw';
        if (ang > 157.5 || ang <= -157.5) return 'left'; if (ang > -157.5 && ang <= -112.5) return 'nw';
        if (ang > -112.5 && ang <= -67.5) return 'up'; return 'ne';
    }
