...
(function() {
    ...
    const SPEED_DELETE_INTERVAL_MS = 100;    
    const SHAKE_BASE_THRESHOLD = 25; 
    const SHAKE_TIMEOUT_MS = 500; 
    
    // --- V3 ENGINE SETTINGS ---
    const FLASH_COOLDOWN_MS = 300; 
    const MOTION_THRESHOLD = 40; // Pixel difference threshold

    // --- AUDIO ENGINE SETTINGS ---
    const AUDIO_DEBOUNCE_MS = 350;
    const MIC_THRESHOLD = 20; 

    const SETTINGS_KEY = 'followMeAppSettings_v9'; // Incremented version
    const STATE_KEY = 'followMeAppState_v9';
    ...
    const SHORTCUT_TRIGGERS = {
        ...
        'longpress_settings': 'Long Press Settings', 'tilt_left': 'Tilt Left', 'tilt_right': 'Tilt Right',
    ...
    const SHORTCUT_ACTIONS = {
        ...
        'clear_all': 'Clear All', 'clear_last': 'Clear Last',
        'toggle_autoplay': 'Toggle Autoplay', 'toggle_audio': 'Toggle Audio',
        'toggle_haptics': 'Toggle Haptics', 'toggle_dark_mode': 'Toggle Dark Mode',
        'open_settings': 'Open Settings', 'open_help': 'Open Help',
        'next_profile': 'Next Profile', 'prev_profile': 'Prev Profile'
    };
    ...
    const VOICE_VALUE_MAP = {
        ...
        'eleven': '11', 'twelve': '12', 'see': 'C', 'dee': 'D', 'e': 'E', 'eff': 'F', 'gee': 'G', 'eh': 'A', 'be': 'B',
    ...
    // V3: Stores previous frame pixel data instead of brightness
    ...
    var shareModal, closeShare, copyLinkButton, nativeShareButton, toastNotification, toastMessage; 
    ...
    function openHelpModal() { generateGeneralHelp(); generateModesHelp(); generatePromptsHelp(); helpModal.classList.remove('opacity-0', 'pointer-events-none'); helpModal.querySelector('div').classList.remove('scale-90'); }
    ...
    // ==========================================================
    // V3 CAMERA ENGINE (MOTION/DELTA LOGIC)
    // ==========================================================
    ...
    function runDetectionLoop() {
        ...
        const buffer = imageData.data; // Raw pixels

        // 2. If no previous frame, save this one and skip
        if (!previousFrameData) {
            previousFrameData = new Uint8ClampedArray(buffer);
            detectionLoopId = requestAnimationFrame(runDetectionLoop);
            return;
        }

        const fr = cameraFeed.getBoundingClientRect(); 
        const gr = activeCalibrationGrid.getBoundingClientRect();
        const sx = detectionCanvas.width / fr.width; 
        const sy = detectionCanvas.height / fr.height;
        const gpx = (gr.left - fr.left) * sx; 
        const gpy = (gr.top - fr.top) * sy;
        const s = getCurrentProfileSettings(); 
        const is12 = (s.currentInput === INPUTS.KEY12);
        const cols = is12 ? 4 : 3; 
        const rows = is12 ? 3 : 3; 
        const boxW = (gr.width * sx) / cols; 
        const boxH = (gr.height * sy) / rows;
        
        const now = Date.now(); 
        // Sensitivity: Lower slider value = MORE sensitive in this logic, so we invert or keep as threshold
        // User's slider is 10-100. Let's treat it as a threshold.
        const threshold = s.flashSensitivity || 40; 

        for (let i = 0; i < cols * rows; i++) {
            const r = Math.floor(i / cols); 
            const c = i % cols;
            // Scan center 50% of box
            const startX = Math.floor(gpx + (c * boxW) + (boxW * 0.25));
            const startY = Math.floor(gpy + (r * boxH) + (boxH * 0.25));
            const endX = startX + Math.floor(boxW * 0.5);
            const endY = startY + Math.floor(boxH * 0.5);

            if (startX < 0 || startY < 0 || endX >= detectionCanvas.width || endY >= detectionCanvas.height) continue;

            let changeCount = 0;
            // Skip pixels for speed (step 4)
            for (let y = startY; y < endY; y += 4) {
                for (let x = startX; x < endX; x += 4) {
                    const pos = (y * detectionCanvas.width + x) * 4;
                    const currG = buffer[pos+1]; // Green channel
                    const prevG = previousFrameData[pos+1];
                    
                    if (Math.abs(currG - prevG) > 30) { // Hard pixel diff
                        changeCount++;
                    }
                }
            }

            // Did enough pixels change?
            if (changeCount > threshold && (now - lastFlashTime[i] > FLASH_COOLDOWN_MS)) {
                if (isCameraMasterOn) addValue(String(i + 1));
                else console.log(`Calibration Hit: ${i+1} (Score: ${changeCount})`);
    ...
    async function startAudioListening() {
        ...
        try {
            ...
            showModal("Mic Error", "Check permissions.", closeModal, "OK", ""); 
    ...
    function runAudioLoop() {
        ...
        if (max > MIC_THRESHOLD) {
            ...
            // Filter for valid range (e.g. 250Hz - 550Hz)
            if (freq > 250 && freq < 550) {
                const m = NOTE_MAP.find(n => Math.abs(freq - n.freq) < 8);
                const now = Date.now();
                if (m && (now - lastAudioHitTime > AUDIO_DEBOUNCE_MS)) { 
                    addValue(m.val); 
                    console.log(`Audio Hit: ${m.val} (${freq.toFixed(0)}Hz)`);
    ...
    function updateAllChrome() {
        ...
        // FIXED: Force buttons to show/hide based on slider
    ...
    // --- HELP TEXT GENERATORS (FIXED TYPOS) ---
    function generateGeneralHelp() {
        const c = document.getElementById('help-tab-general');
        if(c) c.innerHTML = `<h4 class="text-primary-app">App Overview</h4><p>This is a multi-mode sequence tracker...</p>`;
    }
    function generateModesHelp() {
        const c = document.getElementById('help-tab-modes');
        if(c) c.innerHTML = `<h4 class="text-primary-app">Inputs</h4><p>Settings ➔ Profile</p><ul><li>9-Key: 3x3 grid</li><li>12-Key: 4x3 grid</li></ul><h4 class="text-primary-app">Modes</h4><ul><li>Simon Says: Standard</li><li>Unique Rounds: Round based</li></ul>`;
    }
    function generatePromptsHelp() { /* ... same logic as before ... */ }
    ...
    function initializeListeners() {
        document.body.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#mic-master-btn')) { toggleMicMaster(); } // Wired to Audio Toggle
        });
        ...
        const addSettingListener = (el, key, type='value') => {
            if (el) el.addEventListener('input', (e) => {
                ...
                // FIXED: Force UI update for Master Buttons when slider moves
                if(el===autoInputSlider) updateAllChrome(); 
