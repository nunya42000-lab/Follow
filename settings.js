...
const LANG = {
    ...
    es: {
        ...
        blackout_gestures: "Gestos de Pantalla Negra", // NEW TOGGLE LABEL
        help_stealth_detail: "El modo sigilo (1-tecla) simplifica la entrada al asignar los 12 valores primarios (1-12) a una sola pulsación de tecla. La interpretación depende del contexto y del modo (Simon/Único). Está diseñado para una entrada de alta velocidad y movimiento mínimo.", // UPDATED HELP DETAIL
        help_blackout_detail: "El modo de pantalla negra (Blackout) oscurece toda la pantalla para eliminar la distracción visual, permitiéndole concentrarse únicamente en las señales de audio y la memoria muscular. La aplicación sigue siendo completamente funcional, pero la interfaz de usuario está oculta. Si los gestos de pantalla negra están habilitados, la entrada cambia a un sistema táctil 'sin mirar'.", // UPDATED HELP DETAIL
        help_gesture_detail: "Gestos de Pantalla Negra: Un sistema de entrada 'sin mirar'. Utiliza gestos táctiles (deslizamientos, toques) para representar los valores del 1 al 12. Los valores del 6 al 12 se representan con las letras A a G (A=6, B=7, etc.) en una cuadrícula virtual de 3x4. Una guía detallada completa sigue a la Guía de Morse Háptico." // NEW HELP DETAIL
    }
};
...
export class SettingsManager {
    constructor(appSettings, callbacks, sensorEngine) {
        ...
        this.injectBlackoutGesturesToggle(); // NEW CALL
        this.dom = {
    ...
    injectBlackoutGesturesToggle() {
        ...
        // Locate blackout-toggle and insert the new toggle after it
        if (blackoutToggleInput && blackoutToggleInput.parentElement) {
            const row = blackoutToggleInput.parentElement;
            const container = row.parentElement;
            if (container) {
                // Insert after the blackout toggle row
                container.insertBefore(div, row.nextSibling);
                // Update the dom map for immediate use
    ...
    // --- Start of large functions that were previously abbreviated ---
    ...
    openCalibration() {
        if (this.dom.calibModal) {
            ...
            this.sensorEngine.setCalibrationCallback((data) => {
                if (this.dom.calibAudioBar) {
                    const pct = ((data.audio - (-100)) / ((-30) - (-100))) * 100;
                    this.dom.calibAudioBar.style.width = `${Math.max(0, Math.min(100, pct))}%`;
                }
                if (this.dom.calibCamBar) {
                    const pct = Math.min(100, data.camera);
                    this.dom.calibCamBar.style.width = `${pct}%`;
                }
            });
        }
    }
    ...
    closeCalibration() {
        if (this.dom.calibModal) {
            ...
            this.sensorEngine.toggleAudio(this.appSettings.isAudioEnabled);
            this.sensorEngine.toggleCamera(this.appSettings.autoInputMode === 'cam' || this.appSettings.autoInputMode === 'both');
        }
    }
    ...
    initListeners() {
        this.dom.targetBtns.forEach(btn => {
            btn.onclick = () => {
                ...
                const hex = this.tempTheme[this.currentTargetKey];
        ...
        if (this.dom.ftHue) this.dom.ftHue.oninput = () => this.updateColorFromSliders();
        if (this.dom.ftSat) this.dom.ftSat.oninput = () => this.updateColorFromSliders();
        if (this.dom.ftLit) this.dom.ftLit.oninput = () => this.updateColorFromSliders();
        if (this.dom.ftToggle) this.dom.ftToggle.onclick = () => { this.dom.ftContainer.classList.remove('hidden'); this.dom.ftToggle.style.display = 'none'; };
        if (this.dom.edName) this.dom.edName.oninput = (e) => { this.tempTheme.name = e.target.value; };
        if (this.dom.openEditorBtn) this.dom.openEditorBtn.onclick = () => this.openThemeEditor();
        if (this.dom.edCancel) this.dom.edCancel.onclick = () => {
            if (this.dom.editorModal) {
                this.dom.editorModal.querySelector('div').classList.add('scale-90');
                this.dom.editorModal.classList.add('opacity-0');
                setTimeout(() => {
                    this.dom.editorModal.classList.add('pointer-events-none');
                    this.dom.ftContainer.classList.add('hidden');
                    this.dom.ftToggle.style.display = 'block';
                }, 300);
            }
        };
        ...
        if (this.dom.edSave) this.dom.edSave.onclick = () => {
            ...
            if (PREMADE_THEMES[activeId]) {
                const n = prompt("Save As New Theme Name:");
                if (!n) return;
                this.tempTheme.name = n;
        ...
        if (this.dom.themeSelect) this.dom.themeSelect.onchange = (e) => {
        ...
        if (this.dom.themeAdd) this.dom.themeAdd.onclick = () => {
            ...
            if (!n) return;
            const newTheme = { ...PREMADE_THEMES.default, name: n };
            const newId = 'custom_' + Date.now();
            this.appSettings.customThemes[newId] = newTheme;
            this.appSettings.activeTheme = newId;
            this.populateThemeDropdown();
            this.callbacks.onUpdate();
            this.callbacks.onSave();
        ...
        if (this.dom.themeRename) this.dom.themeRename.onclick = () => {
            ...
            if (PREMADE_THEMES[id]) return alert("Cannot rename built-in.");
        ...
        if (this.dom.themeDelete) this.dom.themeDelete.onclick = () => {
            const id = this.appSettings.activeTheme;
            if (PREMADE_THEMES[id]) return alert("Cannot delete built-in.");
        ...
        if (this.dom.configAdd) this.dom.configAdd.onclick = () => {
            ...
            if (!n) return;
            this.callbacks.onProfileAdd(n);
            this.populateConfigDropdown();
            this.callbacks.onSave();
        };

        if (this.dom.configRename) this.dom.configRename.onclick = () => {
            const id = this.appSettings.activeProfileId;
            if (id === 'default') return alert("Cannot rename default profile.");
            const n = prompt("Rename:", this.appSettings.profiles[id].name);
            if (n) {
                this.appSettings.profiles[id].name = n;
                this.populateConfigDropdown();
                this.callbacks.onSave();
            }
        };
        ...
        if (this.dom.configDelete) this.dom.configDelete.onclick = () => {
            const id = this.appSettings.activeProfileId;
            if (id === 'default') return alert("Cannot delete default profile.");
            if (confirm("Delete this profile?")) {
                this.callbacks.onProfileDelete(id);
                this.populateConfigDropdown();
                this.callbacks.onSave();
            }
        };

        if (this.dom.configSave) this.dom.configSave.onclick = () => {
            this.callbacks.onProfileSave();
            alert("Profile Saved!");
        };
        ...
        if (this.dom.voicePresetAdd) this.dom.voicePresetAdd.onclick = () => {
            ...
            const newId = 'custom_' + Date.now();
            if (!this.appSettings.voicePresets) this.appSettings.voicePresets = {};
            this.appSettings.voicePresets[newId] = newPreset;
            this.appSettings.activeVoicePresetId = newId;
        ...
        const bind = (el, prop, isGlobal, isInt = false, isFloat = false) => {
            ...
            el.onchange = () => {
                ...
                else if (isFloat) val = parseFloat(val);
                ...
                this.callbacks.onSave();
                if (prop === 'maxSequenceLength' || prop === 'machines' || prop === 'mode') {
                    this.generatePrompt();
                }
            };
        };
        ...
        bind(this.dom.input, 'inputMode', true);
        bind(this.dom.mode, 'currentMode', false);
        bind(this.dom.machines, 'machines', false, true);
        bind(this.dom.seqLength, 'maxSequenceLength', false, true);
        bind(this.dom.autoClear, 'autoClearInput', true);
        bind(this.dom.longPressToggle, 'isLongPressAutoplayEnabled', true);

        bind(this.dom.hapticMorse, 'isHapticMorseEnabled', true);
        if (this.dom.playbackSpeed) this.dom.playbackSpeed.onchange = (e) => {
            this.appSettings.playbackSpeed = parseFloat(e.target.value);
            this.callbacks.onSave();
            this.generatePrompt();
        };
        bind(this.dom.chunk, 'simonChunkSize', false, true);
        if (this.dom.delay) this.dom.delay.onchange = (e) => {
            this.appSettings.runtimeSettings.simonInterSequenceDelay = parseFloat(e.target.value) * 1000;
            this.callbacks.onSave();
            this.generatePrompt();
        };
        ...
        bind(this.dom.blackoutGesturesToggle, 'isBlackoutGesturesEnabled', true); // NEW BINDING
        ...
        if (this.dom.seqSize) this.dom.seqSize.onchange = (e) => {
            this.appSettings.uiScaleMultiplier = parseInt(e.target.value) / 100.0;
            this.callbacks.onUpdate();
        };
        if (this.dom.gestureMode) this.dom.gestureMode.value = this.appSettings.gestureResizeMode || 'none';
        ...
        if (this.dom.autoInput) this.dom.autoInput.onchange = (e) => {
            this.appSettings.autoInputMode = e.target.value;
            this.callbacks.onSave();
            this.callbacks.onUpdate(); // To update sensor visibility
        };

        if (this.dom.voicePitch) this.dom.voicePitch.oninput = () => { this.appSettings.voicePitch = parseFloat(this.dom.voicePitch.value); this.callbacks.onSave(); };
        if (this.dom.voiceRate) this.dom.voiceRate.oninput = () => { this.appSettings.voiceRate = parseFloat(this.dom.voiceRate.value); this.callbacks.onSave(); };
        if (this.dom.voiceVolume) this.dom.voiceVolume.oninput = () => { this.appSettings.voiceVolume = parseFloat(this.dom.voiceVolume.value); this.callbacks.onSave(); };
        if (this.dom.voiceTestBtn) this.dom.voiceTestBtn.onclick = () => this.testVoice();

        if (this.dom.tabs) this.dom.tabs.forEach(tab => {
            tab.onclick = () => {
                this.dom.tabs.forEach(t => t.classList.remove('active-tab'));
        ...
        if (this.dom.calibAudioSlider) this.dom.calibAudioSlider.oninput = () => {
            const val = parseInt(this.dom.calibAudioSlider.value);
            this.appSettings.sensorAudioThresh = val;
            this.sensorEngine.setSensitivity('audio', val);
            const pct = ((val - (-100)) / ((-30) - (-100))) * 100;
            this.dom.calibAudioMarker.style.left = `${Math.max(0, Math.min(100, pct))}%`;
            this.dom.calibAudioVal.textContent = val + " dB";
        };

        if (this.dom.calibCamSlider) this.dom.calibCamSlider.oninput = () => {
            const val = parseInt(this.dom.calibCamSlider.value);
            this.appSettings.sensorCameraThresh = val;
            this.sensorEngine.setSensitivity('camera', val);
            this.dom.calibCamMarker.style.left = `${Math.max(0, Math.min(100, val))}%`;
            this.dom.calibCamVal.textContent = val + "%";
        };
    ...
    populateConfigDropdown() {
        if (!this.dom.configSelect || !this.dom.quickConfigSelect) return;
        const keys = Object.keys(this.appSettings.profiles);
        ...
        keys.forEach(k => {
            const el = document.createElement('option');
            el.value = k;
            el.textContent = this.appSettings.profiles[k].name;
            this.dom.configSelect.appendChild(el);

            const el2 = document.createElement('option');
            el2.value = k;
            el2.textContent = this.appSettings.profiles[k].name;
            this.dom.quickConfigSelect.appendChild(el2);
        });
    ...
    // Hex to HSL and HSL to Hex utilities
    hexToHsl(hex) {
        let r = 0, g = 0, b = 0;
        if (hex.length === 4) {
            r = "0x" + hex[1] + hex[1];
    ...
    openSetup() {
        if (!this.dom.setupModal) return;
        this.dom.setupModal.classList.remove('opacity-0', 'pointer-events-none');
        setTimeout(() => this.dom.setupModal.querySelector('div').classList.remove('scale-90'), 10);
        this.updateUIFromSettings();
    }
    ...
    updateUIFromSettings() {
        if (this.dom.input) this.dom.input.value = this.appSettings.inputMode || 'key';
        if (this.dom.mode) this.dom.mode.value = this.appSettings.runtimeSettings.currentMode || 'simon';
        if (this.dom.machines) this.dom.machines.value = this.appSettings.runtimeSettings.machines || 1;
        if (this.dom.seqLength) this.dom.seqLength.value = this.appSettings.runtimeSettings.maxSequenceLength || 10;
        if (this.dom.autoClear) this.dom.autoClear.checked = this.appSettings.autoClearInput;
        ...
        if (this.dom.blackoutGesturesToggle) this.dom.blackoutGesturesToggle.checked = this.appSettings.isBlackoutGesturesEnabled; // NEW CHECK
        ...
        if (this.dom.playbackSpeed) this.dom.playbackSpeed.value = this.appSettings.playbackSpeed || 1.0;
        if (this.dom.chunk) this.dom.chunk.value = this.appSettings.runtimeSettings.simonChunkSize || 1;
        if (this.dom.delay) this.dom.delay.value = (this.appSettings.runtimeSettings.simonInterSequenceDelay / 1000) || 0;
        ...
        // Calibration
        if (this.dom.calibAudioSlider) this.dom.calibAudioSlider.value = this.appSettings.sensorAudioThresh || -60;
        if (this.dom.calibCamSlider) this.dom.calibCamSlider.value = this.appSettings.sensorCameraThresh || 50;
        if (this.dom.calibAudioVal) this.dom.calibAudioVal.textContent = (this.appSettings.sensorAudioThresh || -60) + " dB";
        if (this.dom.calibCamVal) this.dom.calibCamVal.textContent = (this.appSettings.sensorCameraThresh || 50) + "%";
        if (this.dom.calibAudioMarker) {
            const val = this.appSettings.sensorAudioThresh || -60;
            const pct = ((val - (-100)) / ((-30) - (-100))) * 100;
            this.dom.calibAudioMarker.style.left = `${Math.max(0, Math.min(100, pct))}%`;
        }
        if (this.dom.calibCamMarker) {
            const val = this.appSettings.sensorCameraThresh || 50;
            this.dom.calibCamMarker.style.left = `${Math.max(0, Math.min(100, val))}%`;
        }
    }
    ...
    generatePrompt() {
        ...
        const max = ps.maxSequenceLength;
        const machines = ps.machines;
        const speed = this.appSettings.playbackSpeed;
        const chunk = ps.simonChunkSize;
        const delay = (ps.simonInterSequenceDelay / 1000).toFixed(1);
        let instructions = '';

        if (machines > 1) {
            // --- MULTI-MACHINE LOGIC ---
            if (ps.currentMode === 'simon') {
                instructions = `MODE: SIMON SAYS (Multi-Machine). - The sequence grows by one number (spread across all ${machines} machines) each round. - I will speak ${machines} numbers, one for each machine, in order (M1, M2, ...). - You must add the new number to the running sequence for each machine. 3. IMMEDIATELY after hearing the numbers, you must READ BACK the sequences for all machines. READBACK RULES (Interleaved Chunking): - Recite the history in chunks of ${chunk}. - Order: Machine 1 (Chunk 1) -> Machine 2 (Chunk 1) -> ... -> Machine 1 (Chunk 2) -> Machine 2 (Chunk 2)... - Pause ${delay} seconds between machine switches.`;
            } else {
                instructions = `MODE: UNIQUE (Random/Non-Repeating, Multi-Machine). - Every round is a fresh set of ${machines} random numbers. - I will speak ${machines} numbers. - You must immediately SORT them: - 1st number -> Machine 1 - 2nd number -> Machine 2 - 3rd number -> Machine 3 (if active), etc. 3. IMMEDIATELY after hearing the numbers, you must READ BACK the sequences for all machines. READBACK RULES (Interleaved Chunking): - Recite the history in chunks of ${chunk}. - Order: Machine 1 (Chunk 1) -> Machine 2 (Chunk 1) -> ... -> Machine 1 (Chunk 2) -> Machine 2 (Chunk 2)... - Do not stop between machines. Flow through the list. - Pause ${delay} seconds between machine switches.`;
            }
        } else {
            // --- SINGLE MACHINE LOGIC ---
            if (ps.currentMode === 'simon') {
                instructions = `MODE: SIMON SAYS (Single Machine). - The sequence grows by one number each round. - I will speak the NEW number. - You must add it to the list and READ BACK the ENTIRE list from the start.`;
            } else {
                instructions = `MODE: UNIQUE (Random/Non-Repeating). - Every round is a fresh random sequence. - I will speak a number. You simply repeat that number to confirm. - Keep a running list. If I say "Review", read the whole list.`;
            }
        }

        const promptText = `Act as a professional Sequence Caller for a memory skill game. You are the "Caller" (App). I am the "Player" (User). SETTINGS: - Max Number: ${max} - Playback Speed: ${speed}x (Speak fast) - Active Machines: ${machines} - Chunk Size: ${chunk} ${instructions} YOUR RULES: 1. Speak clearly but quickly. No fluff, no welcome messages, no intros, no conversation until I say "START". 2. When I say "START", immediately begin the first round with the first number(s). 3. Wait for my response before calling the next number(s). 4. If I make an error, say "ERROR" and then stop.`;
        if (this.dom.promptDisplay) {
            this.dom.promptDisplay.value = promptText;
        }
    }
    // --- End of large functions that were previously abbreviated ---
}
