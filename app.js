...
const DEFAULT_GESTURES = {
    '1': 'swipe_up', '2': 'swipe_right', '3': 'swipe_down', '4': 'swipe_left',
    '5': 'tap', '6': '2f_swipe_up', '7': '2f_swipe_right', '8': '2f_swipe_down',
    '9': '2f_swipe_left', '10': '2f_tap', '11': 'double_tap', '12': 'long_press'
};
...
const DEFAULT_APP = { 
    ...
    showMicBtn: false, showCamBtn: false, autoInputMode: 'none', activeProfileId: 'profile_1', 
    profiles: JSON.parse(JSON.stringify(PREMADE_PROFILES)), runtimeSettings: JSON.parse(JSON.stringify(DEFAULT_PROFILE_SETTINGS)), 
    isPracticeModeEnabled: false, voicePitch: 1.0, voiceRate: 1.0, voiceVolume: 1.0, selectedVoice: null, 
    voicePresets: {}, activeVoicePresetId: 'standard', generalLanguage: 'en',
    gestureMapping: {...DEFAULT_GESTURES}, morseMapping: {...DEFAULT_MORSE}
};
...
function loadState() { 
    try { 
        ...
        if(s) { 
            ...
            // Backfill defaults for new properties
            ...
            if (!appSettings.gestureMapping) appSettings.gestureMapping = {...DEFAULT_GESTURES};
            if (!appSettings.morseMapping) appSettings.morseMapping = {...DEFAULT_MORSE};
...
function vibrateMorse(val) { 
    ...
    // Handle letter inputs if using Piano mode
    const letterMap = { 'A':'6', 'B':'7', 'C':'8', 'D':'9', 'E':'10', 'F':'11', 'G':'12' };
    if (letterMap[val]) key = letterMap[val];

    const patternStr = appSettings.morseMapping[key] || '';
...
function handleBlackoutGestureEnd(e) {
        ...
        if (gestureInputState.maxTouches === 1) {
            ...
            if (gestureInputState.isTapCandidate) {
                ...
                timers.tap = setTimeout(() => {
                    if (gestureInputState.isTapCandidate) {
                        const val = Object.keys(appSettings.gestureMapping).find(k => appSettings.gestureMapping[k] === 'tap');
                        if(val) { addValue(val); vibrateMorse(val); }
                        gestureInputState.isTapCandidate = false;
                    }
                }, 250); 
    ...
    // SWIPE
        ...
        if (gestureInputState.maxTouches === 1) {
            if (!isHorizontal && !isPositive) detectedGesture = 'swipe_up';
            else if (isHorizontal && isPositive) detectedGesture = 'swipe_right';
            else if (!isHorizontal && isPositive) detectedGesture = 'swipe_down';
            else if (isHorizontal && !isPositive) detectedGesture = 'swipe_left';
        } 
        else if (gestureInputState.maxTouches === 2) {
             if (!isHorizontal && !isPositive) detectedGesture = '2f_swipe_up';
            else if (isHorizontal && isPositive) detectedGesture = '2f_swipe_right';
            else if (!isHorizontal && isPositive) detectedGesture = '2f_swipe_down';
            else if (isHorizontal && !isPositive) detectedGesture = '2f_swipe_left';
        }
    }

    // Lookup Value from Mapping
    if (detectedGesture) {
        const val = Object.keys(appSettings.gestureMapping).find(k => appSettings.gestureMapping[k] === detectedGesture);
        if (val) {
            addValue(val);
            vibrateMorse(val);
        }
