// gestures.js
// Version: v71 - High Performance, Linear Bias & Anti-Stuck Safety
...
export class GestureEngine {
    constructor(targetElement, config, callbacks) {
        ...
        this.staleCheckTimer = null; // New Safety Timer
        ...
        // Start safety loop to clear stuck pointers
        this._startStaleCheck();
    }

    _bindHandlers() {
        const t = this.target;
        // passive: false is critical for preventing scroll interference
    ...
    _startStaleCheck() {
        // Run every second to remove pointers older than 2 seconds (Anti-Stuck)
    ...
    _handleUp(e) {
        ...
        if (remaining === 0) {
            ...
            // Reduced debounce from 100 to 50 for snappier response
    ...
    _analyze() {
        ...
        // --- LINEARITY CHECK (Fixes inaccurate shapes/swipes) ---
        // If the path is essentially straight, ignore shapes and force swipe
        ...
        if (fingers === 1) {
            ...
            if (pathLen > this.config.swipeThreshold) {
                ...
                // Only check for shapes if the path is NOT linear
                if (!isLinear) {
        ...
        if (fingers > 1 && type === 'tap' && netDist > 30) {
            type = 'swipe';
            // Linear check for multi-finger too
