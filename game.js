...
export function handleBackspaceStart(event) {
    ...
    // Fix: Multitouch race condition
    if (initialDelayTimer) return;
    ...
    initialDelayTimer = setTimeout(() => {
        ...
        // Default "Hold to Reset" for Unique Rounds if no shortcut mapped
        ...
        if (profileSettings.isSpeedDeletingEnabled) {
            handleBackspace();
...
export function handleBackspaceEnd() {
    ...
    // Fix: Double-toggle bug removed. 
    // The timer now handles the longpress action entirely.
}
...
export function handleSimonDemo() {
    ...
    let i = 0;
    // Fix: Correct animation duration scaling
    const flashDuration = 250 / speedMultiplier; 
