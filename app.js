...
// --- 9-Key Mapping UI fallback ---
(function(){
  const STORAGE_KEY = 'mapping9_v1';
  function loadMappings(){ try{ const v = localStorage.getItem(STORAGE_KEY); return v?JSON.parse(v):null; }catch(e){return null;} }
  function saveMappings(m){ try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(m)); return true;}catch(e){return false;} }
  function defaultMappings(){ const out = {}; for(let i=1;i<=9;i++){ out[i]= { action: 'value_'+i, morse: '' }; } return out; }
  function buildRow(i, container, value){
    ...
    const sel = document.createElement('select'); sel.className='gesture-select';
    const opts = ['value_'+i, 'tap', 'swipe-up', 'swipe-down', 'swipe-left', 'swipe-right', 'long-press', 'none'];
    opts.forEach(o=>{ const e=document.createElement('option'); e.value=o; e.textContent=o; if(value && value.action==o) e.selected=true; sel.appendChild(e); });
    const morse = document.createElement('input'); morse.placeholder='Morse (e.g. .-)'; morse.value = (value && value.morse) ? value.morse : '';
    sel.onchange = morse.onchange = function(){
      const m = loadMappings() || defaultMappings();
      m[i] = { action: sel.value, morse: morse.value };
      saveMappings(m);
    };
    row.appendChild(lbl); row.appendChild(sel); row.appendChild(morse);
    container.appendChild(row);
  }
  function populate9(){
    const container = document.getElementById('mapping-9-container');
    if(!container) return;
    container.innerHTML='';
    const mappings = loadMappings() || defaultMappings();
    for(let i=1;i<=9;i++){ buildRow(i, container, mappings[i]); }
  }
  function wireButtons(){
    const reset = document.getElementById('mapping-reset-btn');
    const exp = document.getElementById('mapping-export-btn');
    const impBtn = document.getElementById('mapping-import-btn');
    const impFile = document.getElementById('mapping-import-file');
    if(reset){ reset.onclick = ()=>{ if(confirm('Reset mappings to defaults?')){ localStorage.removeItem(STORAGE_KEY); populate9(); } }; }
    if(exp){ exp.onclick = ()=>{ const m = loadMappings() || defaultMappings(); const blob = new Blob([JSON.stringify(m, null, 2)], {type:'application/json'}); const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='mapping9.json'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); } }
    if(impBtn && impFile){ impBtn.onclick = ()=> impFile.click(); impFile.onchange = (e)=>{ const f = e.target.files[0]; if(!f) return; const reader = new FileReader(); reader.onload = ()=>{ try{ const obj = JSON.parse(reader.result); if(obj){ localStorage.setItem(STORAGE_KEY, JSON.stringify(obj)); populate9(); alert('Imported'); } }catch(err){ alert('Invalid file'); } }; reader.readAsText(f); }; }
  }
  setTimeout(()=>{
    try{
      if(window.populateMappingUI && typeof window.populateMappingUI === 'function'){
        try{ window.populateMappingUI(); }catch(e){ console.error('populateMappingUI failed',e); populate9(); }
      } else {
        populate9();
      }
      wireButtons();
    }catch(e){
      console.error('Mapping fallback init failed', e);
    }
  }, 400);
})();
