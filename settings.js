    ...
    function populateMappingContainers() {
        try {
            ...
            types.forEach(type => {
                ...
                const count = (type==='9key')?9: (type==='12key')?12:7;
                for(let i=1;i<=count;i++){
                    const row = document.createElement('div'); row.className='flex items-center justify-between';
                    const lbl = document.createElement('div'); lbl.textContent = type + ' - ' + i;
                    const morseInp = document.createElement('input'); morseInp.type='text'; morseInp.className='settings-input p-1 rounded w-1/3 text-sm'; morseInp.value = (window.appSettings.morseMappings && window.appSettings.morseMappings[type] && window.appSettings.morseMappings[type][i])? window.appSettings.morseMappings[type][i] : '';
                    morseInp.dataset.type = type; morseInp.dataset.index = i;
                    morseInp.onchange = function(){ window.appSettings.morseMappings = window.appSettings.morseMappings || {}; window.appSettings.morseMappings[type] = window.appSettings.morseMappings[type] || {}; window.appSettings.morseMappings[type][i] = morseInp.value; if(window.settingsManager && settingsManager.callbacks && settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
                    const gestInp = document.createElement('input'); gestInp.type='text'; gestInp.className='settings-input p-1 rounded w-1/3 text-sm'; gestInp.value = (window.appSettings.gestureMappings && window.appSettings.gestureMappings[type] && window.appSettings.gestureMappings[type][i])? window.appSettings.gestureMappings[type][i] : '';
                    gestInp.dataset.type = type; gestInp.dataset.index = i;
                    gestInp.onchange = function(){ window.appSettings.gestureMappings = window.appSettings.gestureMappings || {}; window.appSettings.gestureMappings[type] = window.appSettings.gestureMappings[type] || {}; window.appSettings.gestureMappings[type][i] = gestInp.value; if(window.settingsManager && settingsManager.callbacks && settingsManager.callbacks.onSave) settingsManager.callbacks.onSave(); };
                    row.appendChild(lbl); row.appendChild(morseInp); row.appendChild(gestInp);
                    list.appendChild(row);
                }
                box.appendChild(list);
                morseC.appendChild(box);
                const box2 = box.cloneNode(true);
                gestureC.appendChild(box2);
            });
