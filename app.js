...
(function() {
    ...
    const DEFAULT_APP_SETTINGS = {
        ...
        isConfirmationsEnabled: true, // <-- NEW SETTING
    ...
    var showConfirmationsToggle = null; // <-- NEW
    ...
    function handleConfigDelete() {
        ...
        if (profileCount <= 1) {
            showModal("Cannot Delete", "You must have at least one configuration.", () => closeModal(), "OK", "", false); // Alert
            return;
        }
    ...
    function openSettingsModal() {
        ...
        showConfirmationsToggle.checked = appSettings.isConfirmationsEnabled; // <-- NEW
    ...
    async function startCameraStream() {
        ...
        try {
            ...
            showModal("Camera Error", `Could not access camera: ${err.message}. Please check browser permissions.`, () => closeModal(), "OK", "", false); // Alert
        }
    }
    ...
    function saveGridConfig() {
        ...
        // --- BUG FIX: Don't save if the grid is collapsed ---
        if (gridRect.width < 10 || gridRect.height < 10) {
            console.warn("Grid is collapsed, not saving config.");
            return;
        }
    ...
    function generateGeneralHelp() {
        // ... (This function is large but unchanged, so I'll snip it for brevity)
...
Let's start with Round 1.`;
    ...
    function showModal(title, message, onConfirm, confirmText = 'OK', cancelText = 'Cancel', isConfirmation = true) {
        // NEW CHECK: If it's a confirmation and confirmations are disabled, just run the action.
    ...
    function handleBackspaceEnd() {
        ...
        if (wasHolding && profileSettings.currentMode === MODES.UNIQUE_ROUNDS && !executeShortcut('longpress_backspace')) {
            showModal('Reset Rounds?', 'Are you sure you want to reset to Round 1?', resetUniqueRoundsMode, 'Reset', 'Cancel', true); // Confirmation
        }
    }
...
const initGridDragger = (gridElement) => {
    ...
    const dragEnd = (e) => {
        ...
        saveGridConfig(); // Save ONLY on drag end
    };
    
    gridElement.addEventListener('mousedown', dragStart);
    gridElement.addEventListener('touchstart', dragStart, { passive: false });
    
    // --- BUG FIX: NO RESIZE OBSERVER ---
    // ResizeObserver is deleted as requested to stop auto-shrink.
    // Resizing via CSS/User action will still trigger the next dragStart/End cycle 
    // or you can add a specific 'mouseup' listener to the grid if you want to capture resize-handle clicks specifically.
    // For now, deleting it is the safest fix.
