...
const LANG = {
    en: {
        ...
        help_blackout_detail: "Boss Mode (Blackout): SHAKE YOUR DEVICE to toggle. Turns the screen black to eliminate distractions. The app remains fully functional.",
        help_gesture_detail: "Gesture Input (🗒️): Tap the Notepad icon in the header to activate a full-screen input pad. Use swipes and taps instead of looking at buttons."
    },
    es: {
        ...
        help_blackout_detail: "Modo Jefe (Blackout): AGITA EL DISPOSITIVO para activar. Oscurece la pantalla para eliminar distracciones.",
        help_gesture_detail: "Entrada de Gestos (🗒️): Toca el icono de Notas en el encabezado para activar el panel de gestos."
    }
};
...
export class SettingsManager {
    ...
    // NEW METHOD: Manages the Auto-Hiding Header Bar
        ...
        // 1. Hide/Show individual buttons
        Object.keys(btns).forEach(k => {
            if(btns[k]) btns[k].classList.toggle('hidden', !visible[k]);
        });

        // 2. STOP THE HEADER FROM HIDING
        // We removed the logic that adds 'header-hidden'. 
        // This keeps the bar stable at the top of the screen.
