...
export class SettingsManager{
    constructor(appSettings,callbacks,sensorEngine){
        ...
        this.dom={
            ...
            // Added explicit references for Quick Toggles
        ...
        // Delay voice populating slightly to ensure browser is ready
    ...
    initListeners(){
        ...
        bind(this.dom.input,'currentInput',false);bind(this.dom.mode,'currentMode',false);bind(this.dom.machines,'machineCount',false,true);bind(this.dom.seqLength,'sequenceLength',false,true);bind(this.dom.autoClear,'isUniqueRoundsAutoClearEnabled',true);
        
        // --- FIXED: Bind Main Autoplay and Sync Quick Start ---
        if(this.dom.autoplay){
            this.dom.autoplay.onchange=(e)=>{
                this.appSettings.isAutoplayEnabled = e.target.checked;
                if(this.dom.quickAutoplay) this.dom.quickAutoplay.checked = e.target.checked; // Sync Quick
                this.callbacks.onSave();
            }
        }
        // --- FIXED: Bind Main Audio and Sync Quick Start ---
        if(this.dom.audio){
            this.dom.audio.onchange=(e)=>{
                this.appSettings.isAudioEnabled = e.target.checked;
                if(this.dom.quickAudio) this.dom.quickAudio.checked = e.target.checked; // Sync Quick
                this.callbacks.onSave();
            }
        }
        // --- FIXED: Bind Quick Start Autoplay ---
        if(this.dom.quickAutoplay){
            this.dom.quickAutoplay.onchange=(e)=>{
                this.appSettings.isAutoplayEnabled = e.target.checked;
                if(this.dom.autoplay) this.dom.autoplay.checked = e.target.checked; // Sync Main
                this.callbacks.onSave();
            }
        }
        // --- FIXED: Bind Quick Start Audio ---
        if(this.dom.quickAudio){
            this.dom.quickAudio.onchange=(e)=>{
                this.appSettings.isAudioEnabled = e.target.checked;
                if(this.dom.audio) this.dom.audio.checked = e.target.checked; // Sync Main
                this.callbacks.onSave();
            }
        }
        // --- FIXED: Bind Quick Start "Don't Show Again" ---
        if(this.dom.dontShowWelcome){
             this.dom.dontShowWelcome.onchange=(e)=>{
                this.appSettings.showWelcomeScreen = !e.target.checked; // Logic inverted: checked means don't show (false)
                if(this.dom.showWelcome) this.dom.showWelcome.checked = !e.target.checked;
                this.callbacks.onSave();
             }
        }
        // --- FIXED: Bind Main "Show Welcome" to sync with Quick Start ---
    ...
    openSetup(){
        ...
        // IMPORTANT: Update UI state before showing to ensure checkboxes are correct
    ...
    updateUIFromSettings(){
        ...
        // --- FIXED: Update Quick Start Toggles ---
