...
/* Styling for the secret button trigger */
#dev-secret-trigger {
    font-weight: 800;
    color: var(--text-main);
...
#dev-secret-trigger:active {
    background: var(--primary-app); /* Flashes your theme color when tapped */
    transform: scale(0.95);
}
/* Developer Modal Overlay */
#developer-modal {
    transition: opacity 0.3s ease-in-out;
    backdrop-filter: blur(8px); /* Gives it that modern frosted glass look */
}
...
#developer-controls-container input[type="range"] {
...
#developer-controls-container input[type="range"]::-webkit-slider-thumb {
    -webkit-appearance: none;
    appearance: none;
    width: 18px;
    height: 18px;
    background: #4f46e5; /* Match your app's primary color */
    border-radius: 50%;
    cursor: pointer;
...
/* --- V2 DEVELOPER EDITION STYLES --- */

/* Full Screen Mode: Add padding so header isn't hidden by status bar */
body.fullscreen-mode #app {
    padding-top: 4rem !important; 
}

/* Gradient behind header for readability in full screen */
body.fullscreen-mode #aux-control-header {
    background: linear-gradient(to bottom, rgba(0, 0, 0, 0.9) 20%, rgba(0, 0, 0, 0) 100%);
}

/* Developer Menu Blur Effect */
...
body.fullscreen-mode #aux-header {
    top: 45px;
}
