...
(function() {
    ...
    const DEFAULT_PROFILE_SETTINGS = {
        ...
        flashSensitivity: 50, 
        // FIX: Defaults set to 75% width/height and centered (12.5% offset)
        cameraGridConfig9: { top: '12.5%', left: '12.5%', width: '75%', height: '75%' },
        cameraGridConfig12: { top: '12.5%', left: '12.5%', width: '75%', height: '75%' }
    };
    ...
    function handleConfigDelete() {
        ...
        showModal(
            ...
            "This will permanently delete this configuration and its saved sequences.",
            () => {
    ...
    function saveGridConfig() {
        ...
        // FIX: Safety check to stop shrinking
        const widthPct = (gridRect.width / containerRect.width) * 100;
        const heightPct = (gridRect.height / containerRect.height) * 100;
        
        // If dimensions are too small (buggy state), do NOT save
        if (widthPct < 20 || heightPct < 20) return;
        ...
        // Ensure DOM reflects state immediately
...
Let's start with Round 1.`;
    ...
    function initializeListeners() {
        ...
        const initGridDragger = (gridElement) => {
            ...
            // --- FIX: REMOVED RESIZEOBSERVER, ADDED MANUAL SAVE ---
    ...
    window.onload = function() {
       ...
       if (!gridElement) return;
            ...
            const dragStart = (e) => {
                // Check for resize handle interaction (bottom-right corner)
                const rect = gridElement.getBoundingClientRect();
                const clientX = e.clientX || e.touches[0].clientX;
                const clientY = e.clientY || e.touches[0].clientY;
                
                // If clicking in the bottom-right 20px, assume resize and do NOT drag
                if (clientX > rect.right - 20 && clientY > rect.bottom - 20) {
                    return; 
                }
